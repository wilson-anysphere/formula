...
- **Awareness sanitization:** awareness identity fields are rewritten to the authenticated user id (`sub` for JWT; `userId` for token introspection). Desktop should use JWT `sub` as its local presence id when available so identities are stable.
