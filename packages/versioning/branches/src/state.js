...
function normalizeSheetView(value) {
  ...
  const normalizeFormatRunsByCol = (raw) => {
    ...
    const addColRuns = (colKey, rawRuns) => {
      ...
      for (const entry of rawRuns) {
        const startRowRaw = entry?.startRow;
        const endRowExclusiveRaw = entry?.endRowExclusive;
        const endRowRaw = entry?.endRow;
        if (startRowRaw == null) continue;
        const endRowExclusiveNum =
          endRowExclusiveRaw != null
            ? Number(endRowExclusiveRaw)
            : endRowRaw != null
              ? Number(endRowRaw) + 1
              : NaN;
        ...
        if (!Number.isInteger(endRowExclusiveNum) || endRowExclusiveNum <= startRow) continue;

        const format = normalizeFormatObject(entry?.format ?? entry?.style);
        if (format == null) continue;

        runs.push({ startRow, endRowExclusive: endRowExclusiveNum, format });
      }

      runs.sort((a, b) => a.startRow - b.startRow);
      if (runs.length === 0) return;
      columns.push({ col, runs });
    };

    if (Array.isArray(raw)) {
      for (const entry of raw) {
        const col = Array.isArray(entry) ? entry[0] : entry?.col ?? entry?.index;
        const runs = Array.isArray(entry) ? entry[1] : entry?.runs;
        addColRuns(col, runs);
      }
    } else if (isRecord(raw)) {
      for (const [key, value] of Object.entries(raw)) {
  ...
  const formatRunsByCol = isRecord(value) ? normalizeFormatRunsByCol(value.formatRunsByCol) : null;
