...
test("Typing '=' suggests top functions (includes =SUM()", async () => {
  const engine = new TabCompletionEngine();
  ...
  assert.ok(suggestions.length > 0, "Expected top-function suggestions for '='");
  assert.ok(
    suggestions.some((s) => s.text === "=SUM("),
    `Expected a =SUM( suggestion, got: ${suggestions.map((s) => s.text).join(", ")}`
  );
});

test("Sheet prefixes are suggested inside range args before '!'", async () => {
  const engine = new TabCompletionEngine({
    schemaProvider: {
      getNamedRanges: () => [],
      getSheetNames: () => ["Sheet1", "Sheet2"],
  ...
  assert.ok(
    suggestions.some((s) => s.text.startsWith(currentInput) && s.text.endsWith("!")),
    `Expected a sheet-prefix suggestion ending with '!', got: ${suggestions.map((s) => s.text).join(", ")}`
  );
  assert.equal(
    suggestions.filter((s) => s.text.endsWith("!)")).length,
    0,
    `Expected sheet-prefix suggestions to not auto-close parens, got: ${suggestions.map((s) => s.text).join(", ")}`
  );
});

test("Quoted sheet prefixes are suggested inside range args (no quote fixing)", async () => {
  const engine = new TabCompletionEngine({
    schemaProvider: {
      getNamedRanges: () => [],
      getSheetNames: () => ["My Sheet"],
  ...
  assert.ok(
    suggestions.some((s) => s.text === "=SUM('My Sheet'!"),
    `Expected a quoted sheet-prefix suggestion, got: ${suggestions.map((s) => s.text).join(", ")}`
  );
  assert.equal(
    suggestions.filter((s) => s.text.endsWith("!)")).length,
    0,
    `Expected quoted sheet-prefix suggestions to not auto-close parens, got: ${suggestions.map((s) => s.text).join(", ")}`
  );
});
