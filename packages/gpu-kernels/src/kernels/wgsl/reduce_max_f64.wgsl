...
fn nan_propagating_max(a: Scalar, b: Scalar) -> Scalar {
  if (isNan(a) || isNan(b)) {
...
fn main(
  ...
  if (idx2 < params.length) {
    let v = input[idx2];
    acc = select(v, nan_propagating_max(acc, v), has);
    has = true;
  }
  ...
  while (stride > 0u) {
    if (local < stride) {
      shared[local] = nan_propagating_max(shared[local], shared[local + stride]);
    }
    workgroupBarrier();
    stride = stride / 2u;
  }
