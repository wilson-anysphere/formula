...
// f32 stored as u32 bits (with CAS-based atomic min).
@group(0) @binding(4) var<storage, read_write> table_mins: array<atomic<u32>>;
...
fn atomic_min_f32(addr: ptr<storage, atomic<u32>, read_write>, value: Scalar) {
  ...
  loop {
    ...
    if (value < old) {
      ...
      // Match JS/TypedArray signed-zero semantics: -0 compares less than +0.
      if (old_bits == 0u && value_bits == 0x8000_0000u) {
        should_update = true;
      }
    }
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
  ...
  atomic_min_f32(&table_mins[slot], v);
}
