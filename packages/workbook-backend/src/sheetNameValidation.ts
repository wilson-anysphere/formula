...
const INVALID_SHEET_NAME_CHAR_SET = new Set<string>(INVALID_SHEET_NAME_CHARACTERS);

export type SheetNameValidationError =
  | { kind: "EmptyName"; message: string }
  | { kind: "TooLong"; message: string; max: number }
  | { kind: "InvalidCharacter"; message: string; character: string }
  | { kind: "LeadingOrTrailingApostrophe"; message: string }
  | { kind: "DuplicateName"; message: string };

export type SheetNameValidationOptions = {
  /**
   * Existing sheet names to check workbook-wide uniqueness (case-insensitive).
   *
   * Note: Provide sheet *names*, not ids.
   */
  existingNames?: Iterable<string>;
  /**
   * Optional existing name to ignore (useful when renaming a sheet to its current name).
   *
   * This is compared using the same case-insensitive normalization as `existingNames`.
   */
  ignoreExistingName?: string | null;
};
...
export function getSheetNameValidationError(
  name: string,
  options: SheetNameValidationOptions = {},
): SheetNameValidationError | null {
...
export function getSheetNameValidationErrorMessage(
  name: string,
  options: SheetNameValidationOptions = {},
): string | null {
