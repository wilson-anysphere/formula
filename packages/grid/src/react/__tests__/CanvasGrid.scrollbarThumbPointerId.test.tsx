...
describe("CanvasGrid scrollbar thumb pointerId gating", () => {
  const rect200 = {
    left: 0,
    top: 0,
    right: 200,
  ...
  it("ignores window pointermove/pointerup events from other pointerIds while dragging the vertical scrollbar thumb", async () => {
    ...
    await act(async () => {
      // Start dragging with pointerId=1.
      vThumb.dispatchEvent(createPointerEvent("pointerdown", { clientX: 0, clientY: 0, pointerId: 1 }));

      // Unrelated pointer events should not affect scroll or end the drag.
      window.dispatchEvent(createPointerEvent("pointermove", { clientX: 0, clientY: 100, pointerId: 2 }));
      window.dispatchEvent(createPointerEvent("pointerup", { clientX: 0, clientY: 100, pointerId: 2 }));
    });

    expect(apiRef.current?.getScroll().y).toBe(0);

    await act(async () => {
      // The original pointer should still be able to move the thumb.
      window.dispatchEvent(createPointerEvent("pointermove", { clientX: 0, clientY: 100, pointerId: 1 }));
      window.dispatchEvent(createPointerEvent("pointerup", { clientX: 0, clientY: 100, pointerId: 1 }));
    });
