...
describe("CanvasGrid pointer interaction perf", () => {
  ...
  it("does not read layout on every pointermove during a drag selection", async () => {
    ...
    await act(async () => {
      root.render(
        <CanvasGrid provider={{ getCell: (row, col) => ({ row, col, value: "" }) }} rowCount={100} colCount={100} />
      );
    });
    ...
    await act(async () => {
      ...
      for (let i = 0; i < 50; i++) {
        selectionCanvas.dispatchEvent(
          createPointerEvent("pointermove", { clientX: 120 + i, clientY: 80, pointerId: 1 })
        );
      }
