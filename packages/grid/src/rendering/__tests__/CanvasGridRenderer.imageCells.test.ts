...
function createJpegHeaderBytesWithLargeMetadata(width: number, height: number): Uint8Array {
  // Create a JPEG header where the SOF0 marker (which contains dimensions) is pushed
  // beyond 256KB by inserting multiple large APP0 segments.
  //
  // This is not a complete/valid JPEG, but it includes enough structure for our
  // dimension parser to extract the advertised size.
  const appLen = 0xffff; // maximum segment length (includes the 2 length bytes)
  const appPayload = appLen - 2;
  const appSegments = 4;
  const sofSegmentTotal = 13; // marker(2) + length(2) + payload(9)
  const total = 2 + appSegments * (2 + appLen) + sofSegmentTotal;
  ...
  for (let i = 0; i < appSegments; i += 1) {
    // APP0 marker
    bytes[o++] = 0xff;
    bytes[o++] = 0xe0;
    // APP0 length (big-endian)
    bytes[o++] = (appLen >> 8) & 0xff;
    bytes[o++] = appLen & 0xff;
    // Payload (zero-filled)
    o += appPayload;
  }
...
describe("CanvasGridRenderer image cells", () => {
  ...
  it("rejects JPEG Blob images with huge dimensions even when the SOF marker is after large metadata", async () => {
    vi.stubGlobal("requestAnimationFrame", (_cb: FrameRequestCallback) => 0);
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "jpeg_bomb_blob_long", altText: "JPEG long header bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    // Push SOF0 beyond 256KB so small header sniffs would miss it.
    const bytes = createJpegHeaderBytesWithLargeMetadata(10_001, 1);
    const imageResolver = vi.fn(async () => new Blob([bytes], { type: "image/jpeg" }));
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("jpeg_bomb_blob_long") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "JPEG long header bomb")).toBe(true);
  });
  ...
  it("rejects SVG images with huge dimensions specified via style without invoking createImageBitmap", async () => {
    vi.stubGlobal("requestAnimationFrame", (_cb: FrameRequestCallback) => 0);
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_style_bomb", altText: "SVG style bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" style="stroke-width: 1; width: 10001px; height: 1px"></svg>`;
    const imageResolver = vi.fn(async () => createSvgBytes(svg));
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_style_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG style bomb")).toBe(true);
  });
  ...
  it("rejects SVG images with huge dimensions specified via <style> block without invoking createImageBitmap", async () => {
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_style_block_bomb", altText: "SVG style block bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_style_block_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG style block bomb")).toBe(true);
  });
  ...
  it("rejects SVG images with huge dimensions specified via calc() style without invoking createImageBitmap", async () => {
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_calc_style_bomb", altText: "SVG calc style bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_calc_style_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG calc style bomb")).toBe(true);
  });

  it("rejects SVG images with huge dimensions specified via calc() multiplication without invoking createImageBitmap", async () => {
    vi.stubGlobal("requestAnimationFrame", (_cb: FrameRequestCallback) => 0);
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_calc_mul_style_bomb", altText: "SVG calc mul style bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" style="width: calc(10001px * 1); height: 1px"></svg>`;
    const imageResolver = vi.fn(async () => createSvgBytes(svg));
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_calc_mul_style_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG calc mul style bomb")).toBe(true);
  });
  ...
  it("rejects SVG images with huge dimensions specified via CSS var() without invoking createImageBitmap", async () => {
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_var_style_bomb", altText: "SVG var style bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_var_style_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG var style bomb")).toBe(true);
  });
  ...
  it("rejects SVG images with huge dimensions specified via calc(var()) without invoking createImageBitmap", async () => {
      ...
      getCell: (row, col) =>
        row === 0 && col === 0
          ? {
              ...
              col,
              value: null,
              image: { imageId: "svg_var_calc_style_bomb", altText: "SVG var calc style bomb", width: 100, height: 50 }
            }
          : null
    };
    ...
    renderer.renderImmediately();
    const pending = (renderer as any).imageBitmapCache.get("svg_var_calc_style_bomb") as
      | { state: "pending"; promise: Promise<void> }
      | { state: string };
    if (pending?.state === "pending") {
      await pending.promise;
    ...
    expect(createImageBitmapSpy).not.toHaveBeenCalled();
    expect(content.rec.drawImages.length).toBe(0);
    expect(content.rec.fillTexts.some((args) => args[0] === "SVG var calc style bomb")).toBe(true);
  });
