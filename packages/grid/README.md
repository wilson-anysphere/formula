...
- `getFillHandleRect(): { x; y; width; height } | null` (viewport coordinates relative to the grid canvases, clipped to the visible viewport. Returns `null` when the handle is not visible, e.g. offscreen or behind frozen rows/cols.)
...
- Zoom is clamped to the range **0.5–3.0**.
