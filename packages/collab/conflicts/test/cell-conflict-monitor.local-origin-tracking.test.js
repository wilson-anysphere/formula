...
test("CellConflictMonitor does not emit conflicts for sequential overwrites when local edits were written via local-origin transactions", () => {
  const docA = new Y.Doc();
  const docB = new Y.Doc();
  ...
  // Local edit via a raw local-origin transaction (no setLocalValue / modifiedBy).
  docA.transact(() => {
    const cell = /** @type {Y.Map<any>} */ (docA.getMap("cells").get(cellKey));
    cell.set("value", "ours");
    cell.set("modified", Date.now());
