/**
 * Perf regression benchmark: CollabSession + binder bulk cell updates.
 *
 * Goal: Catch accidental O(N^2) regressions when a CollabSession-bound
 * DocumentController processes large batches of cell changes.
 *
 * This suite is intentionally **opt-in** (skipped by default) so CI stays fast.
 *
 ...
 *   Running via `pnpm test:node` is recommended so the repo's Node test harness can
 *   enable TypeScript execution when needed.
 *
 * Knobs:
...
perfTest(
  ...
  async () => {
    ...
    session.setPermissions({ role: "editor", userId: "perf-user", rangeRestrictions: [] });
