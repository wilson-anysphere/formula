...
test("CollabSession integration: offline restart merges on reconnect (encrypted local file persistence)", async (t) => {
  const serverDataDir = await mkdtemp(path.join(tmpdir(), "collab-offline-merge-encrypted-server-"));
  const clientDataDir = await mkdtemp(path.join(tmpdir(), "collab-offline-merge-encrypted-client-"));
  ...
  const wsUrl = server.wsUrl;
  const docId = `offline-merge-encrypted-${randomUUID()}`;

  const keyRingJson = KeyRing.create().toJSON();
  const persistenceA1 = new FileCollabPersistence(clientDataDir, {
    compactAfterUpdates: 5,
    keyRing: KeyRing.fromJSON(keyRingJson),
  });
  ...
  // A goes offline, edits, then "restarts".
  sessionA.disconnect();
  sessionA.setCellValue("Sheet1:0:0", "a-offline-encrypted");
  await sessionA.flushLocalPersistence();
  sessionA.destroy();
  sessionA.doc.destroy();
  await persistenceA1.flush(docId);
  ...
  const persistenceA2 = new FileCollabPersistence(clientDataDir, {
    compactAfterUpdates: 5,
    keyRing: KeyRing.fromJSON(keyRingJson),
  });
  sessionA2 = createCollabSession({
  ...
  await sessionA2.whenLocalPersistenceLoaded();
  assert.equal((await sessionA2.getCell("Sheet1:0:0"))?.value, "a-offline-encrypted");

  await sessionA2.whenSynced();

  // B should receive A's offline changes after A reconnects.
  await waitForCondition(
    async () => (await sessionB.getCell("Sheet1:0:0"))?.value === "a-offline-encrypted",
    10_000
  );
  assert.equal((await sessionB.getCell("Sheet1:0:0"))?.value, "a-offline-encrypted");
