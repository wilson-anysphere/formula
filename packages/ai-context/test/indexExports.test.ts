  ...
  it("is importable from TypeScript with full types", () => {
    ...
    // Keep the file next to this test so `../src/index.js` matches real-world usage.
      ...
      [
        ...
        "",
        "// SpreadsheetApi workbook context supports optional includeFormulaValues flags.",
        'void cm.buildWorkbookContextFromSpreadsheetApi({ spreadsheet: { listSheets: () => [] }, workbookId: "wb1", query: "hi", skipIndexing: true, includeFormulaValues: true });',
        'void cm.buildWorkbookContextFromSpreadsheetApi({ spreadsheet: { listSheets: () => [] }, workbookId: "wb1", query: "hi", skipIndexing: true, include_formula_values: true });',
        "",
