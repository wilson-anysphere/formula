...
test("buildWorkbookContext: structured DLP REDACT does not leak malicious dlpHeuristic findings via custom toString (no-op redactor)", async () => {
  const findingSecret = "TopSecretFinding";
  const workbook = {
    id: "wb-dlp-heuristic-finding-tostring",
    sheets: [
      {
        name: "PublicSheet",
        // Needs at least 2 connected non-empty cells for ai-rag region detection to produce a chunk.
  ...
  Object.defineProperty(findingObj, "toString", { value: () => findingSecret, enumerable: false });
  ...
  await indexWorkbook({
    ...
    transform: (record) => ({
      metadata: {
        ...(record.metadata ?? {}),
        dlpHeuristic: { level: "sensitive", findings: [findingObj] },
      },
    }),
  });
  ...
  assert.ok(out.retrieved.length > 0);
  assert.doesNotMatch(JSON.stringify(out.retrieved), new RegExp(findingSecret));
});
