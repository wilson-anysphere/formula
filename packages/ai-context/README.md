...
- under **structured** DLP redaction, replace user-controlled schema/metadata tokens (e.g. sheet/table/namedRange names, chunk titles — including sheet names embedded in A1 ranges / attachment previews) with placeholders so non-heuristic sensitive strings can’t leak even with a no-op redactor
