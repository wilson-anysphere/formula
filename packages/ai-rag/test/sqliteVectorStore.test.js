...
test(
  ...
  async () => {
    ...
    try {
      // Create a database that claims to be v2 (schema_version=2) but is missing the
      // `metadata_hash` column. This can happen when older v2 builds persisted a DB
      ...
      insert.run([
        ...
        5,
        "hello",
        JSON.stringify({ label: "A", metadataHash: "meta-a" }),
      ]);
      insert.free();
      ...
      const hashes = await store.listContentHashes({ workbookId: "wb" });
      assert.equal(hashes.length, 1);
      assert.equal(hashes[0].id, "a");
      assert.equal(hashes[0].contentHash, "hash-a");
      assert.equal(hashes[0].metadataHash, "meta-a");

      const rec = await store.get("a");
      assert.ok(rec);
      assert.equal(rec.metadata.metadataHash, "meta-a");

      // The repair migration should populate the new column and strip metadataHash from metadata_json.
      const migratedStmt = store._db.prepare(
        "SELECT metadata_hash, metadata_json FROM vectors WHERE id = ?;"
      );
      migratedStmt.bind(["a"]);
      assert.ok(migratedStmt.step());
      const migratedRow = migratedStmt.get();
      migratedStmt.free();
      assert.equal(migratedRow[0], "meta-a");
      assert.deepEqual(JSON.parse(migratedRow[1]), { label: "A" });

      await store.close();

      // Ensure the repaired schema persists and can be reopened.
      const store2 = await createSqliteFileVectorStore({ filePath, dimension: 3, autoSave: false });
      const hashes2 = await store2.listContentHashes({ workbookId: "wb" });
      assert.equal(hashes2.length, 1);
      assert.equal(hashes2[0].metadataHash, "meta-a");
      await store2.close();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
    }
...
test(
  "SqliteVectorStore upgrades DB with structured columns but missing schema_version (prefers columns over JSON)",
  { skip: !sqlJsAvailable },
  async () => {
    const createSqliteFileVectorStore = await getCreateSqliteFileVectorStore();
    const tmpRoot = path.join(__dirname, ".tmp");
    await mkdir(tmpRoot, { recursive: true });
    const tmpDir = await mkdtemp(path.join(tmpRoot, "sqlite-store-v2-missing-schema-version-"));
    const filePath = path.join(tmpDir, "vectors.sqlite");
    ...
    try {
      ...
      db.run(`
        ...
        INSERT INTO vector_store_meta (key, value) VALUES ('dimension', '3');
        -- Intentionally omit schema_version to simulate older DBs.

        -- Simulate an older structured schema that predates the metadata_hash column.
        CREATE TABLE IF NOT EXISTS vectors (
          id TEXT PRIMARY KEY,
          workbook_id TEXT,
          vector BLOB NOT NULL,
      ...
      insert.run([
        ...
        5,
        "hello",
        // Include conflicting structured fields to ensure the migration prefers
        // the existing columns when backfilling missing v2 columns.
        JSON.stringify({ sheetName: "WRONG", metadataHash: "mh-old", extra: "X" }),
      ]);
      insert.free();
      ...
      const migratedStmt = store._db.prepare(
        "SELECT sheet_name, metadata_hash, metadata_json FROM vectors WHERE id = ? LIMIT 1;"
      );
      migratedStmt.bind(["old"]);
      assert.ok(migratedStmt.step());
      const migrated = migratedStmt.get();
      migratedStmt.free();
      assert.equal(migrated[0], "Sheet1");
      assert.equal(migrated[1], "mh-old");
      assert.deepEqual(JSON.parse(migrated[2]), { extra: "X" });

      const metaStmt = store._db.prepare(
        "SELECT value FROM vector_store_meta WHERE key = 'schema_version' LIMIT 1;"
      );
      assert.ok(metaStmt.step());
      assert.equal(metaStmt.get()[0], "2");
      ...
      // Reopen to ensure the migration persisted.
      const store2 = await createSqliteFileVectorStore({ filePath, dimension: 3, autoSave: false });
      const rec2 = await store2.get("old");
      assert.ok(rec2);
      assert.equal(rec2.metadata.sheetName, "Sheet1");
      assert.equal(rec2.metadata.metadataHash, "mh-old");
      await store2.close();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
