...
describe("Retention sweep (integration)", () => {
  ...
  it("archives audit logs, deletes versions, purges deleted docs, and respects legal holds", async () => {
    ...
    try {
      ...
      await db.query(
        "INSERT INTO documents (id, org_id, title, created_by) VALUES ($1,$2,$3,$4)",
        [docActive, orgId, "Active", userId]
      );
      await db.query(
        "INSERT INTO documents (id, org_id, title, created_by) VALUES ($1,$2,$3,$4)",
        [docHeld, orgId, "Held", userId]
      );
      await db.query(
      ...
      const onDocumentPurged = vi.fn(async () => {});

      const result = await runRetentionSweep(db, { now, onDocumentPurged });
      ...
      expect(onDocumentPurged).toHaveBeenCalledTimes(1);
      expect(onDocumentPurged).toHaveBeenNthCalledWith(1, { orgId, docId: docDeleted });
      ...
      const archivedAudit = await db.query(
        "SELECT id FROM audit_log_archive ORDER BY created_at ASC"
      );
      ...
      const second = await runRetentionSweep(db, { now, onDocumentPurged });
      ...
      expect(onDocumentPurged).toHaveBeenCalledTimes(2);
      expect(onDocumentPurged).toHaveBeenNthCalledWith(2, { orgId, docId: docDeletedHeld });
      ...
      expect(archivedAfterSecond.rows.map((r) => r.id).sort()).toEqual(
        [aOldActive, aOldHeld].sort()
      );
    } finally {
      await db.end();
    }
  ...
  it("continues when onDocumentPurged throws", async () => {
    const db = await createDb();
    try {
      const now = new Date("2026-01-10T00:00:00.000Z");
      ...
      const result = await runRetentionSweep(db, { now, onDocumentPurged });
      expect(result).toEqual({
        auditEventsArchived: 0,
        documentVersionsDeleted: 0,
        documentsPurged: 1,
        syncPurgesFailed: 1
      });

      const remainingDocs = await db.query("SELECT id FROM documents");
      expect(remainingDocs.rows).toEqual([]);
    } finally {
      await db.end();
    }
  });
