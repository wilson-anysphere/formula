...
describe("KMS (integration)", () => {
  ...
  it("persists local provider state across factory instances", async () => {
    ...
    try {
      ...
      const wrapped = await provider1.wrapKey({ plaintextKey: dek });

      const factory2 = new KmsProviderFactory(db);
      const provider2 = await factory2.forOrg(orgId);
      const unwrapped = await provider2.unwrapKey({ wrappedKey: wrapped });
  ...
  it("rotates keys and preserves ability to decrypt older wrapped keys", async () => {
    ...
    try {
      ...
      const encryptedV1 = await encryptEnvelope({ plaintext, kmsProvider: providerV1 });

      const keyV1 = crypto.randomBytes(32);
      const wrappedV1 = (await providerV1.wrapKey({ plaintextKey: keyV1 })) as any;
      expect(wrappedV1.kmsKeyVersion).toBe(1);

      await rotateOrgKmsKey(db, orgId);

      const providerV2 = await factory.forOrg(orgId);
      const decryptedV1 = await decryptEnvelope({ encryptedEnvelope: encryptedV1, kmsProvider: providerV2 });
      expect(decryptedV1).toEqual(plaintext);

      const keyV2 = crypto.randomBytes(32);
      const wrappedV2 = (await providerV2.wrapKey({ plaintextKey: keyV2 })) as any;
      expect(wrappedV2.kmsKeyVersion).toBe(2);

      expect(await providerV2.unwrapKey({ wrappedKey: wrappedV1 })).toEqual(keyV1);
      expect(await providerV2.unwrapKey({ wrappedKey: wrappedV2 })).toEqual(keyV2);
  ...
  it("rotation sweep rotates orgs only when due", async () => {
    ...
    try {
      ...
      const dek = crypto.randomBytes(32);
      const wrappedV1 = await providerV1.wrapKey({ plaintextKey: dek });
      ...
      expect(first).toEqual({ scanned: 1, rotated: 1, failed: 0, documentVersionDeksRewrapped: 0 });

      const providerAfter = await factory.forOrg(orgId);
      expect(await providerAfter.unwrapKey({ wrappedKey: wrappedV1 })).toEqual(dek);

      const second = await runKmsRotationSweep(db, { now });
      expect(second).toEqual({ scanned: 1, rotated: 0, failed: 0, documentVersionDeksRewrapped: 0 });
