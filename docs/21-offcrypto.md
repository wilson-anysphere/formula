...
| `UnsupportedEncryptedWorkbookKind` | Reserved for future (rare): would indicate the file decrypted successfully, but the decrypted payload is not a supported workbook type. (Formula currently supports decrypted `.xlsx`/`.xlsm`/`.xlsb`.) | Ask the user to confirm the file is a spreadsheet and re-save as `.xlsx`/`.xlsm`/`.xlsb` in Excel (or provide an unencrypted copy). |
