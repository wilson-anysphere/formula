...
1. **`basis` day-count mapping**: must match Excel (`0..=4` with specific 30/360 rules).
   - Note: for `basis=4` (European 30/360), day counts use `DAYS360(..., TRUE)` and the modeled
     coupon-period length `E` follows the same convention: `E = DAYS360(PCD, NCD, TRUE)`. This can
     differ from `360/frequency` for some end-of-month schedules involving February (e.g. Feb 28 →
     Aug 31 yields 182 under European `DAYS360`, not 180).
...
This can differ from `360/frequency` for end-of-month schedules involving February (e.g. Feb 28 →
Aug 31 yields 182 under European `DAYS360`, not 180).
