...
1. **`basis` day-count mapping**: must match Excel (`0..=4` with specific 30/360 rules).
    - Note: for `basis=4` (European 30/360), day counts use `DAYS360(..., TRUE)`, and Excel’s
      ...
      `DAYS360`, while `E` remains 180 for semiannual coupons). Using `DAYS360(PCD, NCD, TRUE)` for
      `E` breaks Excel parity (see `crates/formula-engine/tests/odd_coupon_oracle_regressions.rs`).
...
for semiannual coupons). This behavior is pinned by
`crates/formula-engine/tests/odd_coupon_oracle_regressions.rs`.
