...
- **ODDF\*** enforces strict chronology `issue < settlement < first_coupon <= maturity`.
  - Equality boundaries like `issue == settlement` and `settlement == first_coupon` are rejected with `#NUM!`.
...
1. **`basis` day-count mapping**: must match Excel (`0..=4` with specific 30/360 rules).
   - Note: for `basis=4` (European 30/360), the modeled coupon-period length `E` uses
     `DAYS360(PCD, NCD, TRUE)` between coupon dates (not always `360/frequency`). This can differ from
     `360/frequency` for some end-of-month schedules involving February (e.g. Feb 28 → Aug 31).
