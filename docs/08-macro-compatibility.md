...
- Signature verification includes *binding* the signature to the VBA project contents by extracting
  ...
  - For the `DigitalSignatureExt` stream variant, Office uses the MS-OVBA §2.4.2 v3 transcript and
    ...
    (See `docs/vba-digital-signatures.md` for the `ProjectNormalizedData` transcript definition.)
  ...
  uses the MS-OVBA v3 digest over v3 `ProjectNormalizedData` (i.e. `ContentsHashV3` /
  `V3ContentHash = SHA-256(ProjectNormalizedData)`). Some producers emit inconsistent OIDs (for
  example, an MD5 OID alongside SHA-256 digest bytes), so verifiers should not rely on the OID for
  binding.
...
- From `SignedData.encapContentInfo`, read the `eContent` bytes (if embedded). For Authenticode-like
  ...
  - digest bytes are expected to be:
    ...
    - for the v3 `DigitalSignatureExt` stream (MS-OVBA §2.4.2): 32-byte SHA-256
      (`V3ContentHash` / `ContentsHashV3`). Some producers emit inconsistent OIDs, so verifiers
      should not rely on the OID for binding.
