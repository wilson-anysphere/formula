...
- Signature verification includes *binding* the signature to the VBA project contents by extracting
  ...
  uses the MS-OVBA v3 digest over v3 `ProjectNormalizedData` (typically SHA-256). Some producers emit
  inconsistent OIDs, so verifiers should prefer selecting the digest algorithm by digest length when
  possible.
...
- From `SignedData.encapContentInfo`, read the `eContent` bytes (if embedded). For Authenticode-like
  ...
  - digest bytes are expected to be:
    ...
    - for the v3 `DigitalSignatureExt` stream (MS-OVBA §2.4.2): commonly 32-byte SHA-256 (but some
      producers emit inconsistent OIDs; verifiers should prefer selecting the algorithm by digest
