...
- `bundle.macOS.entitlements` / signing keys and `bundle.windows.timestampUrl`.
  - `bundle.macOS.entitlements` points at `apps/desktop/src-tauri/entitlements.plist`.
    ...
    - Signed bundle guardrail: `bash scripts/validate-macos-bundle.sh` also checks that Hardened Runtime is enabled for signed builds (via `codesign -dv`) and validates the embedded entitlements (via `node scripts/check-macos-entitlements.mjs`).
