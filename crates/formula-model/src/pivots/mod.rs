...
/// Static (definition-level) pivot filter that restricts which items are included.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct FilterField {
    pub source_field: String,
    /// Set of allowed items for this field. `None` means "no filter" (all items allowed).
...
///
/// This is persisted in the workbook model and consumed by computation backends
/// (worksheet/range pivots in `formula-engine`, Data Model pivots in `formula-dax`).
