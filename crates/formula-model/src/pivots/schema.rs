...
fn dax_identifier_requires_quotes(raw: &str) -> bool {
    // DAX table identifiers can be unquoted when they are a simple ASCII identifier. Any
    // whitespace/punctuation (including embedded quotes) requires single-quote quoting.
    //
    // This is intentionally conservative: quoting when unnecessary is still valid DAX.
    let mut chars = raw.chars();
    let Some(first) = chars.next() else {
        return true;
    };
...
fn quote_dax_identifier(raw: &str) -> String {
    // In DAX, quoted identifiers are wrapped in single quotes, and embedded single quotes are
