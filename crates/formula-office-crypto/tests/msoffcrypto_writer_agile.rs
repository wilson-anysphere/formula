...
#[test]
fn agile_tampered_size_header_high_dword_fails_integrity_check() {
    let password = "correct horse battery staple";
    let plaintext = include_bytes!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/xlsx/basic/basic.xlsx"
    ...
    // Extract streams and tamper the EncryptedPackage header high DWORD.
    let mut ole = cfb::CompoundFile::open(Cursor::new(&ole_bytes)).expect("open cfb");
    let mut encryption_info = Vec::new();
    ole.open_stream("EncryptionInfo")
        .expect("open EncryptionInfo")
    ...
    // Some parsers treat the upper 32 bits as reserved, but the HMAC target is the full stream bytes.
