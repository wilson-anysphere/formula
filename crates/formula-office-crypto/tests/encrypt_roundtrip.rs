...
fn tampered_encrypted_hmac_value_fails_integrity_check() {
    ...
    // Mutate the first base64 character of `encryptedHmacValue` to ensure the decoded bytes change
    // while remaining valid base64.
    let xml_bytes = &mut encryption_info[xml_offset..xml_end];
    let needle = b"encryptedHmacValue=\"";
    let pos = xml_bytes
        .windows(needle.len())
        .position(|w| w == needle)
        .expect("encryptedHmacValue attribute not found");
    let value_start = pos + needle.len();
    let value_end = value_start
        + xml_bytes[value_start..]
            .iter()
            .position(|&b| b == b'"')
            .expect("unterminated encryptedHmacValue attribute");
    assert!(
        value_end > value_start,
        "encryptedHmacValue attribute value unexpectedly empty"
    );
