...
fn verify_password_standard_with_key(
    ...
    match header.alg_id {
        CALG_RC4 => {
            // Delegate to the existing verifier logic (RC4 has no IV/cipher mode distinctions).
            let _ = verify_password_standard_with_key_and_mode(header, verifier, hash_alg, key0)?;
            Ok(())
        }
        CALG_AES_128 | CALG_AES_192 | CALG_AES_256 => {
            // MS-OFFCRYPTO Standard AES specifies that verifier fields are encrypted with AES-ECB
            // (no IV). This helper intentionally does **not** apply any CBC fallbacks so callers can
            // layer additional heuristics when needed.
            let verifier_plain = aes_ecb_decrypt(key0, &verifier.encrypted_verifier)?;
            let verifier_hash_plain_full =
                aes_ecb_decrypt(key0, &verifier.encrypted_verifier_hash)?;
