...
fn iterated_hash_sha1(password: &str, salt: &[u8; 16]) -> [u8; 20] {
...
fn block_hash_sha1(h: &[u8; 20], block: u32) -> [u8; 20] {
    let mut hasher = Sha1::new();
    hasher.update(h);
...
fn rc4_key_for_block_sha1(h: &[u8; 20], block: u32, key_size_bits: u32) -> Vec<u8> {
    ...
    let digest = block_hash_sha1(h, block);
...
fn build_standard_rc4_encryption_info(
    ...
    let h = iterated_hash_sha1(password, &salt);
    let key0 = rc4_key_for_block_sha1(&h, 0, key_size_bits);
...
fn build_standard_rc4_encrypted_package(
    ...
    for (block, chunk) in ciphertext.chunks_mut(RC4_BLOCK_LEN).enumerate() {
        let key = rc4_key_for_block_sha1(&h, block as u32, key_size_bits);
...
fn decrypts_standard_rc4_keysize_56_roundtrip() {
    ...
    let encryption_info = build_standard_rc4_encryption_info(password, salt, key_size_bits);
    let encrypted_package =
        build_standard_rc4_encrypted_package(password, salt, key_size_bits, &plaintext);
    let ole_bytes = build_ole_container(&encryption_info, &encrypted_package);

    let decrypted =
        formula_offcrypto::decrypt_standard_ooxml_from_bytes(ole_bytes, password).expect("decrypt");
