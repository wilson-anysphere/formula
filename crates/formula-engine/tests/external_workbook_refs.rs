...
#[test]
fn workbook_only_external_structured_refs_can_be_invalidated_by_sheet_when_metadata_available() {
    let provider = Arc::new(TestExternalProvider::default());
    provider.set_table("Book.xlsx", "Sheet1", table_fixture_multi_col());
    ...
    engine.set_external_refs_volatile(false);
    engine
        .set_cell_formula("Sheet1", "A1", "=SUM([Book.xlsx]Table1[Col2])")
        .unwrap();
    engine.recalculate();
    assert_eq!(engine.get_cell_value("Sheet1", "A1"), Value::Number(60.0));
...
#[test]
fn sum_over_external_table_structured_ref_with_workbook_name_containing_lbracket_resolves() {
    let provider = Arc::new(TestExternalProvider::default());
    provider.set_table("A1[Name.xlsx", "Sheet1", table_fixture_multi_col());

    provider.set("[A1[Name.xlsx]Sheet1", CellAddr { row: 1, col: 1 }, 10.0);
    provider.set("[A1[Name.xlsx]Sheet1", CellAddr { row: 2, col: 1 }, 20.0);
    provider.set("[A1[Name.xlsx]Sheet1", CellAddr { row: 3, col: 1 }, 30.0);

    let mut engine = Engine::new();
    engine.set_external_value_provider(Some(provider));
    engine
        .set_cell_formula("Sheet1", "A1", "=SUM([A1[Name.xlsx]Table1[Col2])")
        .unwrap();
    engine.recalculate();
