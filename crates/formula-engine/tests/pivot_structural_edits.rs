...
#[test]
fn insert_rows_shifts_pivot_registry_when_edit_uses_display_name_and_sheet_has_stable_key() {
    let mut engine = Engine::new();
    seed_sales_data_on(&mut engine, "sheet1_key");
    engine.set_sheet_display_name("sheet1_key", "Sheet1");
    ...
    assert_eq!(entry.destination.start.to_a1(), "D3");
    assert_eq!(entry.destination.end.to_a1(), "E6");
}
