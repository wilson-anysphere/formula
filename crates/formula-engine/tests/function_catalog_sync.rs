...
fn function_catalog_sync() {
    ...
    for entry in catalog.functions {
        ...
        let spec = ComparableFunctionSpec {
            ...
            arg_types: entry.arg_types.iter().map(|t| parse_value_type(t)).collect(),
        };
    ...
    let missing_in_registry: Vec<String> = catalog_names
        .difference(&registry_names)
        .cloned()
        .collect();
    let missing_in_catalog: Vec<String> = registry_names
...
fn function_catalog_mjs_matches_committed_json() {
    ...
    let raw_mjs = raw_mjs
        .strip_prefix("// This file is generated by scripts/generate-function-catalog.js. Do not edit.\n")
        .unwrap_or_else(|| panic!("{} did not start with expected generated header", mjs_path.display()));
    ...
    let mjs_value: JsonValue = serde_json::from_str(raw_mjs)
        .unwrap_or_else(|err| panic!("failed to parse embedded JSON in {}: {err}", mjs_path.display()));

    assert_eq!(
        json_value, mjs_value,
        "{} is out of sync with {}",
        mjs_path.display(),
        json_path.display()
    );
