...
#[cfg(test)]
mod tests {
    use super::{split_external_sheet_key, split_external_sheet_span_key};

    #[test]
    fn split_external_sheet_key_parses_basic_key() {
        assert_eq!(
    ...
    #[test]
    fn split_external_sheet_key_rejects_missing_prefix_brackets() {
    ...
    #[test]
    fn split_external_sheet_key_rejects_empty_parts() {
        assert_eq!(split_external_sheet_key("[]Sheet1"), None);
        assert_eq!(split_external_sheet_key("[Book.xlsx]"), None);
    }

    #[test]
    fn split_external_sheet_key_splits_at_last_closing_bracket() {
        // Workbook ids can include bracket characters (e.g. from a path prefix that contains
        // `[`/`]`). The splitter locates the last closing bracket to recover the workbook id.
        let key = r"[C:\[foo]\Book.xlsx]Sheet1";
        assert_eq!(
    ...
    #[test]
    fn split_external_sheet_span_key_parses_basic_span() {
        assert_eq!(
    ...
    #[test]
    fn split_external_sheet_span_key_splits_at_last_closing_bracket() {
        let key = r"[C:\[foo]\Book.xlsx]Sheet1:Sheet3";
        assert_eq!(
            split_external_sheet_span_key(key),
            Some((r"C:\[foo]\Book.xlsx", "Sheet1", "Sheet3"))
        );
    }

    #[test]
    fn split_external_sheet_span_key_rejects_invalid_spans() {
