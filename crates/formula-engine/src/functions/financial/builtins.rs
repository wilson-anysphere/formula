...
fn eval_date_arg(ctx: &dyn FunctionContext, expr: &CompiledExpr) -> Result<i32, ErrorKind> {
    ...
    match v {
        Value::Error(e) => Err(e),
        Value::Text(s) => {
            date_time::datevalue(&s, ctx.value_locale(), ctx.now_utc(), ctx.date_system())
...
fn oddfprice_fn(ctx: &dyn FunctionContext, args: &[CompiledExpr]) -> Value {
    ...
    let issue = match eval_date_arg(ctx, &args[2]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let first_coupon = match eval_date_arg(ctx, &args[3]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };

    let rate = match eval_number_arg(ctx, &args[4]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let yld = match eval_number_arg(ctx, &args[5]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let redemption = match eval_number_arg(ctx, &args[6]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let frequency = match eval_i32_trunc_arg(ctx, &args[7]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let basis = if args.len() == 9 {
        match eval_i32_trunc_arg(ctx, &args[8]) {
            Ok(v) => v,
            Err(e) => return Value::Error(e),
        }
    } else {
...
fn oddfyield_fn(ctx: &dyn FunctionContext, args: &[CompiledExpr]) -> Value {
    ...
    let pr = match eval_number_arg(ctx, &args[5]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let redemption = match eval_number_arg(ctx, &args[6]) {
...
fn oddlprice_fn(ctx: &dyn FunctionContext, args: &[CompiledExpr]) -> Value {
    ...
    let last_interest = match eval_date_arg(ctx, &args[2]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };

    let rate = match eval_number_arg(ctx, &args[3]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let yld = match eval_number_arg(ctx, &args[4]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let redemption = match eval_number_arg(ctx, &args[5]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let frequency = match eval_i32_trunc_arg(ctx, &args[6]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let basis = if args.len() == 8 {
        match eval_i32_trunc_arg(ctx, &args[7]) {
            Ok(v) => v,
            Err(e) => return Value::Error(e),
        }
    } else {
...
fn oddlyield_fn(ctx: &dyn FunctionContext, args: &[CompiledExpr]) -> Value {
    ...
    let pr = match eval_number_arg(ctx, &args[4]) {
        Ok(v) => v,
        Err(e) => return Value::Error(e),
    };
    let redemption = match eval_number_arg(ctx, &args[5]) {
