...
//!
//! - For **ODDF***, the regular coupon schedule is generated by stepping **backward from `maturity`**
//!   in `12 / frequency` month increments. This preserves Excel’s “anchor date” behavior in cases
//!   where the first coupon date is clamped in shorter months (e.g. maturity on the 30th → first
//!   coupon in February on the 28th/29th).
//! - For **ODDL***, we only need the prior regular coupon date before `last_interest` to compute
//!   `E`; we step backward from `last_interest` by `12 / frequency` months.
//! - In both cases, if the anchor date is an end-of-month (EOM) date, all stepped coupon dates are
//!   pinned to EOM (equivalent to `EOMONTH` stepping).
//!
//! ---
//!
