...
impl<'a> CompileCtx<'a> {
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        // Normalize the index to an integer. Excel's CHOOSE uses truncation semantics; INT matches
        // truncation for non-negative values (and out-of-range negative values error either way).
        self.program
            .instrs
            .push(Instruction::new(OpCode::LoadLocal, idx_local, 0));
        let int_func_idx = intern_func(self.program, Function::Int);
        self.program
            .instrs
            .push(Instruction::new(OpCode::CallFunc, int_func_idx, 1));
        self.program
            .instrs
            .push(Instruction::new(OpCode::StoreLocal, idx_local, 0));

        // If the index is an error, return it without evaluating any choices.
        self.program
            .instrs
            .push(Instruction::new(OpCode::LoadLocal, idx_local, 0));
        ...
        for (idx, choice) in choices.iter().enumerate() {
            // For the first case, the index value is already on the stack from the `JumpIfNotError`
            // probe above. Subsequent cases reload it.
            if idx != 0 {
                self.program
                    .instrs
                    .push(Instruction::new(OpCode::LoadLocal, idx_local, 0));
        ...
        // If index is out-of-range, CHOOSE returns #VALUE! without evaluating any branches.
        self.push_error_const(ErrorKind::Value);
        let end_target = self.program.instrs.len() as u32;
