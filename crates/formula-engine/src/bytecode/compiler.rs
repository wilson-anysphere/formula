...
impl<'a> CompileCtx<'a> {
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        // Evaluate index once and coerce to an integer (Excel semantics: truncate toward zero for
        // positive numbers, which matches INT for the valid index domain).
        self.compile_expr(&args[0]);
        let int_fn_idx = intern_func(self.program, Function::Int);
        self.program
            .instrs
            .push(Instruction::new(OpCode::CallFunc, int_fn_idx, 1));
        ...
        for (idx, choice) in args[1..].iter().enumerate() {
            ...
            // Match branch: evaluate the selected choice and jump to the end.
            self.compile_expr(choice);
            let jump_end_idx = self.program.instrs.len();
            self.program
                .instrs
                .push(Instruction::new(OpCode::Jump, 0, 0));
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        // Evaluate and normalize the index once. Excel truncates the coerced number.
        // The bytecode runtime lacks a dedicated truncation opcode, so reuse INT which is
        // equivalent for all in-range indices (>= 1).
        self.compile_expr(&args[0]);
        ...
        // Bounds check: 1 <= idx <= choices.len()
        let choices_len = (args.len() - 1) as f64;
        let one_const = self.program.consts.len() as u32;
        self.program
            .consts
            .push(ConstValue::Value(Value::Number(1.0)));
        let max_const = self.program.consts.len() as u32;
        self.program
            .consts
            .push(ConstValue::Value(Value::Number(choices_len)));
        ...
        self.program
            .instrs
            .push(Instruction::new(OpCode::PushConst, one_const, 0));
        self.program.instrs.push(Instruction::new(OpCode::Ge, 0, 0));
        let ge_jump_idx = self.program.instrs.len();
        self.program
            .instrs
            .push(Instruction::new(OpCode::JumpIfFalseOrError, 0, 0));
        ...
        self.program
            .instrs
            .push(Instruction::new(OpCode::PushConst, max_const, 0));
        self.program.instrs.push(Instruction::new(OpCode::Le, 0, 0));
        let le_jump_idx = self.program.instrs.len();
        self.program
            .instrs
            .push(Instruction::new(OpCode::JumpIfFalseOrError, 0, 0));

        // Dispatch on idx.
        let mut case_jump_indices: Vec<usize> = Vec::new();
        let mut end_jump_indices: Vec<usize> = Vec::new();

        for (choice_i, choice_expr) in args[1..].iter().enumerate() {
            let idx_value = (choice_i + 1) as f64;
            let idx_const = self.program.consts.len() as u32;
            self.program
                .consts
                .push(ConstValue::Value(Value::Number(idx_value)));
            ...
            self.program
                .instrs
                .push(Instruction::new(OpCode::PushConst, idx_const, 0));
            self.program.instrs.push(Instruction::new(OpCode::Eq, 0, 0));

            let case_jump_idx = self.program.instrs.len();
            self.program
                .instrs
                .push(Instruction::new(OpCode::JumpIfFalseOrError, 0, 0));
            case_jump_indices.push(case_jump_idx);

            // Selected branch.
            self.compile_expr(choice_expr);
            let jump_end_idx = self.program.instrs.len();
            self.program
                .instrs
                .push(Instruction::new(OpCode::Jump, 0, 0));
            end_jump_indices.push(jump_end_idx);

            // False branch: continue to next case.
            let next_target = self.program.instrs.len() as u32;
            self.program.instrs[case_jump_idx] =
                Instruction::new(OpCode::JumpIfFalseOrError, next_target, 0);
        }

        // Should be unreachable due to bounds checks, but keep Excel semantics and return #VALUE.
        let value_err_target = self.program.instrs.len() as u32;
        self.push_error_const(ErrorKind::Value);

        let end_target = self.program.instrs.len() as u32;

        // Patch bounds-check jumps: FALSE -> #VALUE, ERROR -> end (propagate).
        self.program.instrs[ge_jump_idx] =
            Instruction::new(OpCode::JumpIfFalseOrError, value_err_target, end_target);
        self.program.instrs[le_jump_idx] =
            Instruction::new(OpCode::JumpIfFalseOrError, value_err_target, end_target);

        // Patch case jumps: ERROR -> end_target (propagate).
        for idx in case_jump_indices {
            let a = self.program.instrs[idx].a();
            self.program.instrs[idx] = Instruction::new(OpCode::JumpIfFalseOrError, a, end_target);
        }

        // Patch end jumps from successful branches.
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        // CHOOSE(index, value1, value2, ...)
        //
        // Excel evaluates only the selected value expression (lazy). Implement with explicit
        // control flow so the VM can short-circuit the unused branches.
        if args.len() < 2 {
            self.push_error_const(ErrorKind::Value);
            return;
        }

        // Evaluate the index expression once and store it in a temp local.
        self.compile_expr_inner(&args[0], false);
        ...
        // Error path: reload the error and jump to the end.
        self.program
            .instrs
            .push(Instruction::new(OpCode::LoadLocal, index_local, 0));
        let jump_error_end_idx = self.program.instrs.len();
        ...
        // Coerce the index to an integer-ish value. Excel uses truncation; INT uses floor, but both
        // are equivalent for all valid CHOOSE indices (>= 1). Using INT also matches the runtime's
        // numeric coercion rules.
        // Note: `JumpIfNotError` does not pop the non-error value, so at this point the index
        // value is still on the stack.
        let int_func = intern_func(self.program, Function::Int);
        self.program
            .instrs
            .push(Instruction::new(OpCode::CallFunc, int_func, 1));
        let int_index_local = self.alloc_temp_local("\u{0}CHOOSE_INT_INDEX");
        self.program
            .instrs
            .push(Instruction::new(OpCode::StoreLocal, int_index_local, 0));

        let mut jump_if_idxs: Vec<usize> = Vec::new();
        let mut jump_end_idxs: Vec<usize> = vec![jump_error_end_idx];

        // Cases are 1-based, corresponding to args[1..].
        for (case_idx, choice) in args[1..].iter().enumerate() {
            // Compare INT(index) == (case_idx+1).
            self.program
                .instrs
                .push(Instruction::new(OpCode::LoadLocal, int_index_local, 0));
            let idx_const = self.program.consts.len() as u32;
            self.program
                .consts
                .push(ConstValue::Value(Value::Number((case_idx + 1) as f64)));
            self.program
                .instrs
                .push(Instruction::new(OpCode::PushConst, idx_const, 0));
            self.program.instrs.push(Instruction::new(OpCode::Eq, 0, 0));
            ...
            // Match branch: evaluate the selected value expression (allowing ranges so CHOOSE can
            // return references for use in aggregate/lookup contexts).
            self.compile_expr_inner(choice, true);
            let jump_end_idx = self.program.instrs.len();
            self.program
                .instrs
                .push(Instruction::new(OpCode::Jump, 0, 0));
        ...
        // No match (including idx < 1 or idx > choices): CHOOSE returns #VALUE!.
        self.push_error_const(ErrorKind::Value);
        let end_target = self.program.instrs.len() as u32;
