...
impl<'a> CompileCtx<'a> {
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        // Evaluate the index exactly once, then coerce it to an integer. Excel truncates the
        // coerced number; `INT` matches that behavior for the valid (positive) index domain.
