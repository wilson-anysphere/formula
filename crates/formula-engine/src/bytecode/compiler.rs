...
impl<'a> CompileCtx<'a> {
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        let mut error_jump_indices: Vec<usize> = vec![ge_jump_idx, le_jump_idx];
        ...
        for (choice_i, choice_expr) in args[1..].iter().enumerate() {
            ...
            error_jump_indices.push(case_jump_idx);
        ...
        // Patch bounds-check jumps to the #VALUE error block.
        ...
        // Patch case jumps: error -> end_target.
        for idx in error_jump_indices.into_iter().skip(2) {
            // `a` is already patched; set `b` for error propagation.
        ...
        for jump_idx in end_jump_indices {
            self.program.instrs[jump_idx] = Instruction::new(OpCode::Jump, end_target, 0);
        }
    }
    ...
    fn compile_choose(&mut self, args: &[Expr]) {
        ...
        // Evaluate the index expression once, then truncate toward zero by calling ROUNDDOWN(x, 0).
        // `INT` is floor-based (incorrect for negative indices), and the VM has no trunc opcode.
        self.compile_expr_inner(&args[0], false);
        ...
        for (idx, choice_expr) in args[1..].iter().enumerate() {
            // Compare truncated index == (idx+1).
            self.program
                .instrs
                .push(Instruction::new(OpCode::LoadLocal, index_local, 0));

            let choice_idx = self.program.consts.len() as u32;
            self.program
                .consts
                .push(ConstValue::Value(Value::Number((idx + 1) as f64)));
            self.program
                .instrs
                .push(Instruction::new(OpCode::PushConst, choice_idx, 0));
            self.program.instrs.push(Instruction::new(OpCode::Eq, 0, 0));
        ...
        // No match (including out-of-range indices): CHOOSE returns #VALUE!.
        self.push_error_const(ErrorKind::Value);
        let end_target = self.program.instrs.len() as u32;
