...
impl XlsbFixtureBuilder {
    ...
    pub fn add_shared_string_with_rich_runs_and_phonetic(
        &mut self,
        s: &str,
        runs: Vec<u8>,
        phonetic_tail: Vec<u8>,
    ) -> u32 {
        const STR_RUN_LEN: usize = 8;
        assert!(
            runs.len() % STR_RUN_LEN == 0,
        ...
        let idx = self.shared_strings.len() as u32;
        let c_run = (runs.len() / STR_RUN_LEN) as u32;
        let mut extra = Vec::with_capacity(4 + runs.len() + phonetic_tail.len());
        extra.extend_from_slice(&c_run.to_le_bytes());
        extra.extend_from_slice(&runs);
        extra.extend_from_slice(&phonetic_tail);
