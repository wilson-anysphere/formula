...
#[test]
fn encodes_tableless_this_row_structured_ref_outside_table_range_when_single_table() {
    let mut builder = XlsbFixtureBuilder::new();
...
"#;
    ...
    // Base cell is outside the table range, but the workbook contains exactly one table so the
    // encoder can still infer the intended table id for `[@Qty]`.
    let base = CellCoord::new(20, 0); // A21
    assert_eq!(ctx.single_table_id(), Some(1));
