...
#[test]
fn calcchain_bin_relationship_id_renumbering_is_warning_by_default_and_critical_in_strict_mode() {
    let expected_zip = zip_bytes(&[(
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/calcChain" Target="calcChain.bin"/>
    ...
    let actual_zip = zip_bytes(&[(
        ...
        br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId5" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/calcChain" Target="calcChain.bin"/>
</Relationships>"#,
    )]);
    ...
    assert!(
        report
            .differences
            .iter()
            .any(|d| d.part == "xl/_rels/workbook.bin.rels"
                && d.kind == "relationship_id_changed"
                && d.severity == Severity::Warning),
        "expected calcChain.bin relationship_id_changed diff to be WARNING by default, got {:#?}",
        report.differences
    );
    ...
    assert!(
        report
            ...
            .any(|d| d.part == "xl/_rels/workbook.bin.rels"
                && d.kind == "relationship_id_changed"
                && d.severity == Severity::Critical),
        "expected calcChain.bin relationship_id_changed diff to be CRITICAL in strict mode, got {:#?}",
        report.differences
    );
}
...
#[test]
fn calcchain_bin_related_rels_and_content_types_remain_critical_when_strict_enabled() {
    let expected_zip = zip_bytes(&[
        (
            "[Content_Types].xml",
            br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    assert!(
        report.differences.iter().any(|d| d.kind == "missing_part"
            && d.part == "xl/calcChain.bin"
            && d.severity == Severity::Critical),
        "expected xl/calcChain.bin missing_part to be CRITICAL in strict mode, got {:#?}",
        report.differences
    );
    assert!(
        report
            ...
            .iter()
            .any(|d| d.part == "[Content_Types].xml" && d.severity == Severity::Critical),
        "expected [Content_Types].xml calcChain.bin diffs to be CRITICAL in strict mode, got {:#?}",
        report.differences
    );
    assert!(
        report
            .differences
            .iter()
            .any(|d| d.part.ends_with(".rels") && d.severity == Severity::Critical),
        "expected *.rels calcChain.bin diffs to be CRITICAL in strict mode, got {:#?}",
        report.differences
    );
}
