...
fn patch_workbook_bin_set_first_sheet_visibility(workbook_bin: &[u8], visibility_flags: u32) -> Vec<u8> {
    ...
    loop {
        ...
        if id == SHEET_RECORD_ID {
            out[payload_start..payload_start + 4].copy_from_slice(&visibility_flags.to_le_bytes());
            return out;
        }
