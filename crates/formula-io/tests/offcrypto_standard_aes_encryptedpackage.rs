...
fn decrypts_real_excel_standard_aes_encryptedpackage_fixture() {
    ...
    // `formula-io`'s Standard AES `EncryptedPackage` decryptor supports both:
    // - baseline AES-ECB (Excel), and
    // - a non-standard segmented AES-CBC variant (fallback mode when a salt is provided).
    //
    // This fixture is generated by real Excel and decrypts via the ECB path.
    let decrypted =
