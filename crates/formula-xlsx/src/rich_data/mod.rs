...
impl XlsxPackage {
    /// Extract in-cell images stored via Excel RichData / rich values.
    ///
    /// This is a best-effort extractor that supports both:
    /// - `richValue*.xml`-backed rich values (preferred when present)
    /// - legacy/degenerate packages where the rich value index maps directly to
    ///   `richValueRel.xml` relationship entries.
    ///
    /// Missing optional parts return `Ok(empty)`. Invalid XML returns an error.
    pub fn extract_rich_data_images(
        &self,
    ) -> Result<HashMap<(String, CellRef), Vec<u8>>, XlsxError> {
        // If we can't resolve sheet names to worksheet parts, treat it as "no rich data".
        if self.part("xl/workbook.xml").is_none()
            || self.part("xl/_rels/workbook.xml.rels").is_none()
        {
            return Ok(HashMap::new());
        }

        // Worksheet list parsing can still fail for malformed workbook.xml; bubble that up.
        let worksheet_parts = self.worksheet_parts()?;
        ...
        let vm_to_rich_value = parse_vm_to_rich_value_index_map(metadata_bytes)
            .map_err(rich_data_error_to_xlsx_error)?;
        if vm_to_rich_value.is_empty() {
            return Ok(HashMap::new());
        }

        let Some(rich_value_rel_bytes) = self.part("xl/richData/richValueRel.xml") else {
            return Ok(HashMap::new());
        };
        ...
        let Some(rich_value_rel_rels_bytes) =
            self.part("xl/richData/_rels/richValueRel.xml.rels")
        else {
            return Ok(HashMap::new());
        };
        ...
        // Prefer richValue parts when present (full-fidelity); otherwise fall back to the
        // historical interpretation where the "rich value index" is also the relationship index.
        let mut rich_value_parts: Vec<&str> = self
            .part_names()
            .filter(|name| is_rich_value_part(name))
            .collect();
        rich_value_parts.sort_by(|a, b| cmp_rich_value_parts_by_numeric_suffix(a, b));
        let rich_value_to_rel_index = if rich_value_parts.is_empty() {
            None
        } else {
            Some(
                parse_rich_value_parts_rel_indices(self, &rich_value_parts)
                    .map_err(rich_data_error_to_xlsx_error)?,
            )
        };
        ...
        for sheet in worksheet_parts {
            ...
            // Best-effort: skip malformed worksheet parts instead of failing the whole extraction.
            let cells = match parse_worksheet_vm_cells(sheet_bytes, &sheet.worksheet_part) {
                Ok(cells) => cells,
                Err(_) => continue,
            };
            for (cell, vm) in cells {
                ...
                let rel_index = match &rich_value_to_rel_index {
                    Some(map) => match map.get(&rich_value_idx) {
                        Some(&idx) => idx,
                        None => continue,
                    },
                    None => rich_value_idx,
...
fn rich_data_error_to_xlsx_error(err: RichDataError) -> XlsxError {
    match err {
