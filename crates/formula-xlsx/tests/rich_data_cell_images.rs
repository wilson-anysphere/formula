...
fn build_rdrichvalue_image_xlsx_with_reordered_structure() -> Vec<u8> {
    let workbook_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
...
<metadata xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:xlrd="http://schemas.microsoft.com/office/spreadsheetml/2017/richdata">
  ...
  <extLst>
    ...
    // Reordered keys: CalcOrigin comes before the `_rvRel:LocalImageIdentifier` relationship slot.
    let rd_rich_value_structure = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/>
    ...
    zip.start_file("xl/richData/rdrichvaluestructure.xml", options)
        .unwrap();
    zip.write_all(rd_rich_value_structure.as_bytes()).unwrap();

    zip.start_file("xl/richData/rdrichvalue.xml", options).unwrap();
    ...
    zip.start_file("xl/media/image1.png", options).unwrap();
    zip.write_all(b"rdrichpng").unwrap();

    zip.finish().unwrap().into_inner()
}

fn build_rich_image_xlsx_with_metadata1_referenced_from_workbook_rels() -> Vec<u8> {
    let workbook_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/>
    ...
    zip.start_file("xl/metadata1.xml", options).unwrap();
    zip.write_all(metadata_xml.as_bytes()).unwrap();
...
fn build_rich_image_xlsx_with_rich_value_rel1_referenced_from_rich_value_rels() -> Vec<u8> {
    let workbook_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rIdRichValueRel" Type="http://example.com/relationships/richValueRel" Target="richValueRel1.xml"/>
</Relationships>"#;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/>
    ...
    // Non-canonical richValueRel part name.
    zip.start_file("xl/richData/richValueRel1.xml", options)
        .unwrap();
    zip.write_all(rich_value_rel_xml.as_bytes()).unwrap();

    zip.start_file("xl/richData/_rels/richValueRel1.xml.rels", options)
        .unwrap();
    zip.write_all(rich_value_rel_rels.as_bytes()).unwrap();
...
#[test]
fn extracts_rich_cell_image_bytes_from_rdrichvalue_reordered_structure() {
    let bytes = build_rdrichvalue_image_xlsx_with_reordered_structure();
    let pkg = XlsxPackage::from_bytes(&bytes).expect("read pkg");
    let images = pkg.extract_rich_cell_images_by_cell().expect("extract images");
...
#[test]
fn extracts_images_when_metadata_part_is_numbered_and_referenced_from_workbook_rels() {
    let bytes = build_rich_image_xlsx_with_metadata1_referenced_from_workbook_rels();
    let pkg = XlsxPackage::from_bytes(&bytes).expect("read pkg");
    let images = pkg.extract_rich_cell_images_by_cell().expect("extract images");
...
#[test]
fn extracts_images_when_rich_value_rel_part_is_numbered_and_referenced_from_rich_value_rels() {
    let bytes = build_rich_image_xlsx_with_rich_value_rel1_referenced_from_rich_value_rels();
    let pkg = XlsxPackage::from_bytes(&bytes).expect("read pkg");
    let images = pkg.extract_rich_cell_images_by_cell().expect("extract images");
