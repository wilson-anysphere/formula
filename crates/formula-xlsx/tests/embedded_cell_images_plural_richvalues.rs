...
#[test]
fn extract_embedded_cell_images_accepts_plural_richvalues_part_name() {
    // Construct a minimal package that forces `extract_embedded_cell_images` to map:
    //
    //   worksheet vm=1 -> metadata rich value index 1 -> richValues.xml[1] -> rel slot 0
    //
    // This catches regressions where we only scan `richValue*.xml` (singular) and would therefore
    // fail to find the rich value record for index 1.
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1"
    ...
    let cell_img = images.get(&key).expect("expected embedded image at A1");
    assert_eq!(cell_img.image_part, "xl/media/image1.png");
    assert_eq!(cell_img.image_bytes, image_bytes);
}
