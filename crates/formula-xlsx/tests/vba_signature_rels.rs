...
fn build_signature_container(signature: &[u8]) -> Vec<u8> {
    let cursor = Cursor::new(Vec::new());
    let mut ole = cfb::CompoundFile::create(cursor).expect("create compound file");
    {
        // `cfb` finalizes stream metadata (like length) when the stream handle is dropped, so keep
        // the stream in a smaller scope before extracting the underlying bytes.
        let mut stream = ole
...
fn verify_vba_signature_is_resolved_via_vba_project_relationships() {
    ...
    let sig_container = build_signature_container(&pkcs7);
