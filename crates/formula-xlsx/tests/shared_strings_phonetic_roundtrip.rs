...
const PHONETIC_MARKER_DUP_A: &str = "PHO_DUP_A";
const PHONETIC_MARKER_DUP_B: &str = "PHO_DUP_B";
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2">
  <si>
    <t>Base</t>
    <rPh sb="0" eb="4"><t>{PHONETIC_MARKER_DUP_A}</t></rPh>
  </si>
  <si>
    <t>Base</t>
    <rPh sb="0" eb="4"><t>{PHONETIC_MARKER_DUP_B}</t></rPh>
  </si>
</sst>"#
    );
...
#[test]
fn patch_pipeline_preserves_shared_string_index_when_text_unchanged(
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    patches.set_cell(
        "Sheet1",
        CellRef::from_a1("A2")?,
        CellPatch::set_value(CellValue::String("Base".to_string())),
    );
    pkg.apply_cell_patches(&patches)?;
    ...
    assert_eq!(
        v, "1",
        "expected patcher to preserve the original shared string index when text is unchanged"
    );
...
#[test]
fn streaming_patch_preserves_shared_string_index_when_text_unchanged(
) -> Result<(), Box<dyn std::error::Error>> {
    let bytes = build_duplicate_shared_strings_xlsx();
    ...
    assert_eq!(
        v, "1",
        "expected streaming patcher to preserve the original shared string index when text is unchanged"
    );
