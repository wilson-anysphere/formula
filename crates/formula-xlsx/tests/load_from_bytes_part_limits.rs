...
use formula_xlsx::{load_from_bytes, ReadError, XlsxError};
use zip::ZipArchive;
...
#[test]
fn load_from_bytes_rejects_oversized_worksheet_parts_before_inflation() {
    // Build a valid workbook, then forge the worksheet part's *declared* uncompressed size to
    // exceed the per-part inflation cap. `load_from_bytes` (which inflates all parts) should
    // reject this deterministically before attempting a huge allocation.
    let mut workbook = Workbook::new();
    workbook.add_sheet("Sheet1").unwrap();

    let mut out = Cursor::new(Vec::new());
    formula_xlsx::write_workbook_to_writer(&workbook, &mut out).expect("write workbook");
    let mut bytes = out.into_inner();

    // Sanity: the part exists.
    {
        let mut zip = ZipArchive::new(Cursor::new(bytes.as_slice())).expect("open zip");
        zip.by_name("xl/worksheets/sheet1.xml")
            .expect("expected sheet1.xml");
    }

    bytes = patch_zip_entry_uncompressed_size(bytes, "xl/worksheets/sheet1.xml", 1_000_000_000);
    let err = load_from_bytes(&bytes).expect_err("expected oversized worksheet to error");
    match err {
