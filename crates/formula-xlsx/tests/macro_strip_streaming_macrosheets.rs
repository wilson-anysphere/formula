...
use formula_xlsx::{strip_vba_project_streaming, validate_opc_relationships, XlsxPackage};
...
#[test]
fn streaming_macro_stripping_removes_macrosheets_and_dialogsheets() {
    let fixture = build_macrosheet_fixture();
    let mut out = Cursor::new(Vec::new());

    strip_vba_project_streaming(Cursor::new(fixture), &mut out).expect("strip macros");

    let pkg = XlsxPackage::from_bytes(&out.into_inner()).expect("read stripped package");

    assert!(pkg.part("xl/vbaProject.bin").is_none());
    assert!(pkg.part("xl/macrosheets/sheet2.xml").is_none());
    assert!(pkg.part("xl/dialogsheets/sheet3.xml").is_none());
    assert!(pkg.part("xl/macrosheets/_rels/sheet2.xml.rels").is_none());
    assert!(pkg.part("xl/dialogsheets/_rels/sheet3.xml.rels").is_none());
    ...
    validate_opc_relationships(pkg.parts_map())
