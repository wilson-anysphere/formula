...
use formula_xlsx::print::{read_workbook_print_settings, PrintError};
use zip::ZipArchive;
...
#[test]
fn read_workbook_print_settings_rejects_oversized_workbook_xml() {
    ...
    // Sanity check: workbook.xml exists.
    {
    ...
    let err = read_workbook_print_settings(&bytes).expect_err("expected oversized workbook.xml");
    match err {
        ...
        other => panic!("expected PrintError::PartTooLarge, got {other:?}"),
    }
}
