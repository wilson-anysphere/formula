...
#[test]
fn pivot_cache_definition_for_cache_id_falls_back_to_filename_when_workbook_is_malformed() {
    let bytes = build_synthetic_package_with_malformed_workbook();
    let pkg = XlsxPackage::from_bytes(&bytes).expect("read pkg");
...
<pivotCacheDefinition xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cacheFields count="0"/>
    ...
    for (name, xml) in [
        ("xl/workbook.xml", workbook_xml),
        ("xl/_rels/workbook.xml.rels", workbook_rels),
        ("xl/pivotCache/pivotCacheDefinition7.xml", cache_definition_xml),
    ] {
        zip.start_file(name, options).unwrap();
        zip.write_all(xml.as_bytes()).unwrap();
    }
...
fn build_synthetic_package_with_malformed_workbook() -> Vec<u8> {
    let cache_definition_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<pivotCacheDefinition xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <cacheSource type="worksheet">
    <worksheetSource ref="A1:C5" sheet="Sheet1"/>
  ...
  <cacheFields count="0"/>
    ...
    // A malformed workbook that should not cause cacheId lookups to error.
