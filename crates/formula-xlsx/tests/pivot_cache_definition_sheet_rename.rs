...
fn build_source_package_with_interleaved_chartsheet() -> Vec<u8> {
    let content_types = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
  <Default Extension="xml" ContentType="application/xml"/>
  ...
  <Override PartName="/xl/pivotCache/pivotCacheRecords1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.pivotCacheRecords+xml"/>
    ...
    // Workbook sheet list: Sheet1 (worksheet), Chart1 (chartsheet), Sheet2 (worksheet).
    let workbook = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
 xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <pivotCaches>
...
<chartsheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"/>"#;
    ...
    // Use the trailing worksheet (`Sheet2`) as the pivot cache worksheetSource. This sheet will be
    // renamed in the destination package.
    let cache_def = br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><pivotCacheDefinition xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cacheSource type="worksheet"><worksheetSource sheet="Sheet2" ref="Sheet2!A1:C5"/></cacheSource></pivotCacheDefinition>"#;
    let cache_records = br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><pivotCacheRecords xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="0"/>"#;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotCacheRecords" Target="pivotCacheRecords1.xml"/>
    ...
    zip.start_file("xl/chartsheets/sheet1.xml", options).unwrap();
    zip.write_all(chartsheet).unwrap();
...
fn build_destination_package_with_chartsheet_and_sheet_rename() -> Vec<u8> {
    // Destination workbook sheet list: Sheet1 (worksheet), Data (renamed worksheet), Chart1
    // (chartsheet appended).
    let content_types = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  <Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
...
#[test]
fn preserved_pivot_cache_definition_sheet_rename_ignores_interleaved_chartsheets() {
    let source_bytes = build_source_package_with_interleaved_chartsheet();
    let source_pkg = XlsxPackage::from_bytes(&source_bytes).expect("read source package");
    let preserved = source_pkg
        .preserve_pivot_parts()
        .expect("preserve pivot parts");

    let dest_bytes = build_destination_package_with_chartsheet_and_sheet_rename();
    let mut dest_pkg = XlsxPackage::from_bytes(&dest_bytes).expect("read destination");
    ...
    assert!(
        !cache_def_xml.contains(r#"sheet="Sheet2""#),
        "expected old worksheetSource sheet name to be removed: {cache_def_xml}"
    );
    assert!(
        cache_def_xml.contains(r#"ref="Data!A1:C5""#),
