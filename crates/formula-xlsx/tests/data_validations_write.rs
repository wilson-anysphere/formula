...
#[test]
fn no_op_roundtrip_preserves_data_validations_with_case_variants_and_empty_formula2(
) -> Result<(), Box<dyn std::error::Error>> {
    // Some producers emit mixed-case `type=` / `operator=` values and may include a self-closing
    // `<formula2/>` element. Our semantic-change detector should still treat these as equivalent to
    // the model representation so no-op saves preserve the original bytes.
    let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData/>
  <dataValidations count="1">
    <dataValidation sqref="A1" type="Whole" operator="Between" allowBlank="1" showInputMessage="0" showErrorMessage="0">
      <formula1>1</formula1>
      <formula2/>
    </dataValidation>
  </dataValidations>
