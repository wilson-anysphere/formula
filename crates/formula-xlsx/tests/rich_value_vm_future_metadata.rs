...
"#;
    ...
    // Two metadataTypes to ensure `rc/@t` is interpreted as a 1-based index into `<metadataTypes>`,
    // like real Excel files (XLRICHVALUE is the second entry => t="2").
...
"#;
    ...
    // `vm="2"` here is intentionally 1-based to ensure the parser handles index base ambiguity.
    // It points at the second `<valueMetadata><bk>` which (via futureMetadata indirection)
    // resolves to rich value index 42.
...
#[test]
fn loads_rich_value_indices_from_future_metadata_vm() -> Result<(), Box<dyn std::error::Error>> {
