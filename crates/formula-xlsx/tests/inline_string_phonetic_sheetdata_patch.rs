...
#[test]
fn sheetdata_patch_preserves_inline_string_phonetic_runs_on_style_only_edit(
    ...
    let base_t = is_node
        ...
        .expect("expected visible inline string <t>");
    assert_eq!(base_t.text(), Some("Base"));

    let rph = is_node
        .descendants()
        .find(|n| n.is_element() && n.tag_name().name() == "rPh")
        .expect("expected phonetic <rPh> subtree to be preserved");
    let rph_t = rph
        .descendants()
        .find(|n| n.is_element() && n.tag_name().name() == "t")
        .and_then(|n| n.text())
        .unwrap_or("");
    assert_eq!(rph_t, PHONETIC_MARKER);
