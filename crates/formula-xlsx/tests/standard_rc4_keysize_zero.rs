...
//! `formula-xlsx` should accept such files and decrypt them correctly.
...
fn build_minimal_zip() -> Vec<u8> {
    // We intentionally create a small but valid ZIP container; Standard decryption validates the
    // output is a plausible ZIP to reduce false positives when probing decryption schemes.
...
fn decrypts_standard_cryptoapi_rc4_with_keysize_zero() {
    ...
    let decrypted = decrypt_ooxml_encrypted_package(&encryption_info, &encrypted_package, PASSWORD)
        .expect("decrypt RC4 keySize=0 file");
    assert_eq!(decrypted, plaintext, "decrypted bytes must match plaintext");

    let err =
        decrypt_ooxml_encrypted_package(&encryption_info, &encrypted_package, "wrong-password")
