...
#[test]
fn embedded_cell_images_supports_custom_rich_value_rel_part_name() {
    // Some producers use a custom part name for the rich value relationship table (e.g.
    // `customRichValueRel.xml`) instead of the canonical `richValueRel.xml` / `richValueRel1.xml`.
    // Ensure `extract_embedded_cell_images` can still discover and use it.
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/>
    ...
    let bytes = build_package(&[
        ...
        // Custom rich value rel part name (not the canonical richValueRel.xml).
        ("xl/richData/customRichValueRel.xml", rich_value_rel_xml),
        (
