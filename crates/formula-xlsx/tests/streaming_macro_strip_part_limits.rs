...
use formula_xlsx::{strip_vba_project_streaming, write_workbook_to_writer, StreamingPatchError, XlsxError};
use zip::ZipArchive;
...
#[test]
fn strip_vba_project_streaming_rejects_oversized_content_types_part() {
    // Build a small XLSX then forge `[Content_Types].xml` metadata to claim an enormous uncompressed
    // size. The macro-strip streaming pipeline reads and rewrites content types; it must fail fast
    // with PartTooLarge rather than attempting a large allocation.
    ...
    bytes = patch_zip_entry_uncompressed_size(bytes, "[Content_Types].xml", 1_000_000_000);
    let err = strip_vba_project_streaming(Cursor::new(bytes), Cursor::new(Vec::new()))
        .expect_err("expected strip_vba_project_streaming to reject oversized content types");
    match err {
        StreamingPatchError::Xlsx(XlsxError::PartTooLarge { part, size, max }) => {
            assert!(part.eq_ignore_ascii_case("[Content_Types].xml"));
