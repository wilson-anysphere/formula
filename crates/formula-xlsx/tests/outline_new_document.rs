...
#[test]
fn new_document_writes_outline_rows_and_cols() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Sheet XML should include outline attributes, even for rows/cols without cells.
    ...
    assert!(
        ...
        "expected <cols> to be written when outline column metadata is present"
    );
    ...
    let row3 = parsed
        ...
        .expect("row 3 exists");
    assert_eq!(row3.attribute("outlineLevel"), Some("1"));
    ...
    assert_eq!(row4.attribute("hidden"), None);
    ...
    let col_c = parsed
        .descendants()
        .find(|n| col_covers(*n, 3))
        .expect("col C exists");
    assert_eq!(col_c.attribute("outlineLevel"), Some("1"));
    ...
    assert_eq!(col_d.attribute("hidden"), None);

    // Validate with the outline parser too (includes outline-hidden derivation).
    let outline = formula_xlsx::outline::read_outline_from_xlsx_bytes(
        &saved,
        "xl/worksheets/sheet1.xml",
    )?;
    assert_eq!(outline.rows.entry(2).level, 1);
    assert!(outline.rows.entry(2).hidden.outline);
    assert_eq!(outline.rows.entry(4).collapsed, true);
    assert_eq!(outline.cols.entry(2).level, 1);
    assert!(outline.cols.entry(2).hidden.outline);
    assert_eq!(outline.cols.entry(4).collapsed, true);
