...
#[test]
fn load_data_model_infers_page_size_rows_when_schema_json_page_size_is_too_large() {
    let tmp = NamedTempFile::new().expect("tmpfile");
    let path = tmp.path();
    ...
    // If `page_size_rows` is larger than the encoded chunk size, columnar access can panic by
    // indexing past the end of the first chunk. The loader should infer a consistent page size
    // from the stored chunks instead.
    let conn = Connection::open(path).expect("open raw db");
    let workbook_id_str = workbook.id.to_string();
    let schema_json = r#"{"version":1,"page_size_rows":3,"cache_max_entries":4}"#;
    conn.execute(
        "UPDATE data_model_tables SET schema_json = ?1 WHERE workbook_id = ?2 AND name = ?3",
        params![schema_json, &workbook_id_str, "Fact"],
    )
