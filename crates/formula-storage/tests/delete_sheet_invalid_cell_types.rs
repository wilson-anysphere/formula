...
#[test]
fn delete_sheet_skips_corrupt_formula_cells() {
    let uri = "file:delete_sheet_invalid_cell_types?mode=memory&cache=shared";
    ...
    // Insert a corrupt formula cell with an invalid row type; delete should ignore it rather than failing.
    conn.execute(
        r#"
    ...
    assert_eq!(cells[0].1.formula.as_deref(), Some("#REF!"));
}
