    ...
    fn decode_rgce_is_robust(rgce in proptest::collection::vec(any::<u8>(), 0..=256)) {
        ...
        match res.unwrap() {
            ...
            Err(e) => {
                ...
                if let Some(ptg) = e.ptg() {
                    prop_assert_eq!(ptg, rgce[offset]);
                }
            }
        }
    }
...
fn operator_stack_underflow_is_error() {
    ...
    let rgce = vec![0x03];
    let err = decode_rgce(&rgce).unwrap_err();
    assert!(matches!(err, DecodeRgceError::StackUnderflow { .. }));
    assert_eq!(err.offset(), 0);
...
fn trailing_bytes_after_complete_expression_is_error() {
    ...
    let err = decode_rgce(&rgce).unwrap_err();
    assert!(matches!(err, DecodeRgceError::StackNotSingular { stack_len: 2, .. }));
