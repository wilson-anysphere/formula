...
mod filepass_tests {
    ...
    #[test]
    fn decrypts_rc4_cryptoapi_legacy_filepass_layout_allows_empty_password() {
        // Legacy BIFF8 RC4 CryptoAPI files can be encrypted with an empty password. Ensure we accept
        // `""` rather than treating it as "password missing".
        const RECORD_BOF: u16 = 0x0809;
        const RECORD_EOF: u16 = 0x000A;
        const RECORD_INTERFACEHDR: u16 = 0x00E1;
        const RECORD_BOUNDSHEET8: u16 = 0x0085;
        ...
        let key0 =
            derive_block_key_spec_padded_to_16(hash_alg, &key_material, 0, effective_key_size_bits);
