...
#[test]
fn imports_note_comment_when_txo_cch_text_is_zero_and_second_fragment_missing_flags_byte() {
    let bytes =
        xls_fixture_builder::build_note_comment_txo_cch_text_zero_missing_flags_in_second_fragment_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("NotesTxoCchZeroNoFlagsMid")
        .expect("NotesTxoCchZeroNoFlagsMid missing");
