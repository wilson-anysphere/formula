...
#[test]
fn imports_shared_formulas_when_shrfmla_ref8_header_omits_cuse() {
    let bytes = xls_fixture_builder::build_shared_formula_shrfmla_ref8_no_cuse_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("SharedRef8NoCuse")
        .expect("SharedRef8NoCuse sheet missing");
