...
#[test]
fn imports_shrfmla_only_shared_formula_ptgarea_row_oob_as_ref_error() {
    ...
    let bytes =
        xls_fixture_builder::build_shared_formula_ptgarea_row_oob_shrfmla_only_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("SharedAreaRowOOB_ShrFmlaOnly")
        .expect("SharedAreaRowOOB_ShrFmlaOnly missing");

    let base = sheet
        .formula(CellRef::from_a1("B65535").unwrap())
        .expect("expected formula in SharedAreaRowOOB_ShrFmlaOnly!B65535");
    assert_eq!(base, "SUM(A65535:A65536)+1");
    assert_parseable_formula(base);

    let follower = sheet
        .formula(CellRef::from_a1("B65536").unwrap())
        .expect("expected formula in SharedAreaRowOOB_ShrFmlaOnly!B65536");
