...
#[test]
fn imports_biff8_autofilter12_future_record_continued_via_continuefrt12() {
    let bytes = xls_fixture_builder::build_autofilter12_continuefrt12_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("Filter12Cont")
        .expect("Filter12Cont missing");
