...
#[test]
fn imports_shrfmla_only_shared_formula_with_ptgref_relative_flags() {
    let bytes =
        xls_fixture_builder::build_shared_formula_shrfmla_only_ptgref_relative_flags_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("SharedOnlyRefFlags")
        .expect("SharedOnlyRefFlags missing");
