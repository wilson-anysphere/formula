...
#[test]
fn imports_shared_3d_ref_oob_from_shrfmla_only() {
    ...
    let bytes = xls_fixture_builder::build_shared_formula_3d_oob_shrfmla_only_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("Shared3D_OOB_ShrFmlaOnly")
        .expect("Shared3D_OOB_ShrFmlaOnly missing");

    let base = sheet
        .formula(CellRef::from_a1("B65535").unwrap())
        .expect("expected formula in Shared3D_OOB_ShrFmlaOnly!B65535");
    assert_eq!(base, "Sheet1!A65536+1");
    assert_parseable_formula(base);

    let follower = sheet
        .formula(CellRef::from_a1("B65536").unwrap())
        .expect("expected formula in Shared3D_OOB_ShrFmlaOnly!B65536");
