...
#[test]
fn builtin_defined_name_prefers_rgchname_when_chkey_mismatches() {
    ...
    // The fixture stores a mismatched built-in id in `rgchName` for the Sheet1 Print_Area name, but
    // populates `NAME.chKey` with a different value. Excel appears to treat `chKey` as a keyboard
    // shortcut for built-in names, so the importer should prefer `rgchName` when present.
