...
#[test]
fn recovers_ptgexp_shared_formula_when_shrfmla_missing() {
    let bytes = xls_fixture_builder::build_shared_formula_ptgexp_missing_shrfmla_fixture_xls();
    ...
    let formula = sheet.formula(b2).expect("expected formula in Shared!B2");
