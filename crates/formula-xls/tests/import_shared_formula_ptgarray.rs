...
#[test]
fn imports_shared_formula_with_ptgarray_constant_and_rgcb() {
    let bytes = xls_fixture_builder::build_shared_formula_ptgarray_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("SharedArray")
        .expect("SharedArray missing");

    let b1 = CellRef::from_a1("B1").unwrap();
    let b2 = CellRef::from_a1("B2").unwrap();

    let f1_opt = sheet.formula(b1);
    let f2_opt = sheet.formula(b2);
    let f1 = f1_opt.expect("B1 formula missing");
    let f2 = f2_opt.expect("B2 formula missing");
    ...
    // Ensure the shared-formula expansion occurred (relative ref A1 -> A2).
    assert!(
        f1.contains("A1"),
