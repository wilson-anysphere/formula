...
#[test]
fn imports_autofilter_sort_state_from_sort12_continued_via_continuefrt12() {
    let bytes = xls_fixture_builder::build_autofilter_sort12_continuefrt12_fixture_xls();
    let result = import_fixture(&bytes);

    let sheet = result
        .workbook
        .sheet_by_name("FilterSort12Cont")
        .expect("FilterSort12Cont missing");
