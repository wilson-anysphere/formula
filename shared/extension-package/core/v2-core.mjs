// ESM wrapper for the v2 extension package core logic.
//
// `shared/` is CommonJS (see `shared/package.json`), but the browser verifier
// (`shared/extension-package/v2-browser.mjs`) is ESM so it can run in webviews.
// Vite dev servers treat workspace `shared/**.js` files as source modules and do
// not automatically apply CommonJS transforms, so importing `v2-core.js` (CJS)
// directly fails at runtime.
//
