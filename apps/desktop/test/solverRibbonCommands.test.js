...
test("Ribbon schema includes Formulas → Solver command id", () => {
  const schema = readRibbonSchemaSource("formulasTab.ts");

  assert.match(schema, /\bid:\s*["']formulas\.solutions\.solver["']/);
  assert.match(schema, /\btestId:\s*["']ribbon-solver["']/);
});

test("Solver ribbon command is registered in CommandRegistry (no exemptions / no main.ts switch cases)", () => {
  ...
  const id = "formulas.solutions.solver";
  assert.match(
    builtins,
