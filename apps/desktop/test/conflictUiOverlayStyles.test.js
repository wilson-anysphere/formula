...
test("spreadsheet conflict UI overlay uses CSS classes (no static inline styles)", async () => {
  ...
  assert.ok(!text.includes("this.conflictUiContainer.style.position"), "expected conflict overlay position to be driven by CSS");
  ...
  assert.ok(!text.includes("structuralToastRoot.style."), "expected structural conflict toast root to avoid static inline styles");
  assert.ok(!text.includes("structuralDialogRoot.style."), "expected structural conflict dialog root to avoid static inline styles");

  // Sanity: ensure the expected CSS classes are applied.
  assert.match(text, /conflictUiContainer\.classList\.add\("conflict-ui-overlay"\)/);
  assert.match(text, /toastRoot\.classList\.add\("conflict-ui-toast-root"\)/);
  assert.match(text, /dialogRoot\.classList\.add\("conflict-ui-dialog-root"\)/);
  assert.match(text, /structuralToastRoot\.classList\.add\("structural-conflict-ui-toast-root"\)/);
  assert.match(text, /structuralDialogRoot\.classList\.add\("structural-conflict-ui-dialog-root"\)/);
});
