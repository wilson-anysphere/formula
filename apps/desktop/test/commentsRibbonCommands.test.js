...
test("Desktop main.ts syncs Comments pressed state + dispatches via CommandRegistry", () => {
  ...
  assert.doesNotMatch(router, /\btoggleOverrides\s*[:=]\s*\{[\s\S]*?["']comments\.togglePanel["']\s*:/m);
  assert.doesNotMatch(router, /\bcommandOverrides\s*[:=]\s*\{[\s\S]*?["']comments\.togglePanel["']\s*:/m);
  assert.doesNotMatch(router, /\bcommandOverrides\s*[:=]\s*\{[\s\S]*?["']comments\.addComment["']\s*:/m);
  assert.doesNotMatch(
    router,
