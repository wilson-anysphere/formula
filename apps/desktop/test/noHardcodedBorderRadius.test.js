...
test("desktop UI should not hardcode border-radius values (use radius tokens)", () => {
  ...
  // Ensure that any border-radius variables referenced by border-radius declarations do not hide hardcoded
  // unit lengths, even if those variables are defined in a different stylesheet. Include transitive references so
  // `border-radius: var(--a)` cannot hide `--a: var(--b)` + `--b: 4px`.
  /** @type {Set<string>} */
  const expandedBorderRadiusVarRefs = new Set(globalBorderRadiusVarRefs);
