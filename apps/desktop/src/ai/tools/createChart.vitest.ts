...
describe("create_chart desktop integration", () => {
  ...
  it("does not mutate the live chart store when previewing create_chart", async () => {
    const controller = new DocumentController();
    controller.setCellValue("Sheet1", "A1", "Category");
    controller.setCellValue("Sheet1", "B1", "Value");
    controller.setCellValue("Sheet1", "A2", "A");
    ...
    const preview = await previewEngine.generatePreview(
      [
        {
          name: "create_chart",
          parameters: { chart_type: "bar", data_range: "A1:B3", title: "Sales" }
        }
      ],
      api,
    ...
    expect(chartStore.listCharts()).toHaveLength(0);
  });
});
