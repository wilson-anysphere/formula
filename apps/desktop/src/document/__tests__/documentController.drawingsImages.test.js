...
test("applyState accepts singleton object-wrapped bytes arrays (interop)", () => {
  const snapshot = new TextEncoder().encode(
    JSON.stringify({
      schemaVersion: 1,
      sheets: [
      ...
      images: [{ id: "img1", mimeType: "image/png", bytes: { 0: [1, 2, 3] } }],
    }),
  );
...
test("applyState accepts singleton object-wrapped bytes base64 strings (interop)", () => {
  const snapshot = new TextEncoder().encode(
    JSON.stringify({
      schemaVersion: 1,
      sheets: [
      ...
      images: [{ id: "img1", mimeType: "image/png", bytes: { 0: "AQID" } }],
    }),
  );
...
test("applyState accepts singleton-wrapped images arrays (interop)", () => {
  const snapshot = new TextEncoder().encode(
    JSON.stringify({
      schemaVersion: 1,
      sheets: [
      ...
      images: [[{ id: "img1", bytes: [1, 2, 3], mimeType: "image/png" }]],
    }),
  );
...
test("applyState accepts singleton object-wrapped images arrays (interop)", () => {
  const snapshot = new TextEncoder().encode(
    JSON.stringify({
      schemaVersion: 1,
      sheets: [
...
test("applyState accepts singleton-wrapped sheet.view.drawings arrays (interop)", () => {
  const drawing = {
    id: "d_view",
    zOrder: 4,
    kind: { type: "image", imageId: "img_view" },
...
test("applyExternalDrawingDeltas accepts singleton-wrapped drawings arrays (interop)", () => {
  const doc = new DocumentController();
