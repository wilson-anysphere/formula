...
describe("SpreadsheetApp stale sheet id safety", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  it("does not resurrect a deleted sheet during refresh when app.sheetId is stale", () => {
    ...
    try {
      ...
      // Simulate an internal stale sheet pointer (e.g. during sheet deletion/undo/applyState races).
      (app as any).sheetId = "Sheet2";

      // Historically, refresh paths could call `document.getSheetView/getCell` and recreate the sheet.
      app.refresh("full");
