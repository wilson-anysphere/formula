...
describe("SpreadsheetApp Excel-style date/time insertion shortcuts (serial values + number format)", () => {
  ...
  it("inserts the current date into the selection with Ctrl+;", () => {
    vi.useFakeTimers();
  ...
  it("inserts the current time into the selection with Ctrl+Shift+;", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2020, 0, 2, 3, 4, 5));
  ...
  it("Insert Date/Time commands no-op while the formula bar is actively editing", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2020, 0, 2, 3, 4, 5));
  ...
  it("commits formula bar edits with Enter while focus is on the grid", () => {
    const root = createRoot();
    const status = {
      activeCell: document.createElement("div"),
      selectionRange: document.createElement("div"),
  ...
  it("cancels formula bar edits with Escape while focus is on the grid", () => {
    const root = createRoot();
    const status = {
      activeCell: document.createElement("div"),
      selectionRange: document.createElement("div"),
