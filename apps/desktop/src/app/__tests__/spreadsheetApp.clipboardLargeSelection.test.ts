...
describe("SpreadsheetApp clipboard large selections", () => {
  beforeEach(() => {
  ...
  it("does not allocate clipboard payloads for very large selections", async () => {
    ...
    // 5000x100 = 500,000 cells (well above MAX_CLIPBOARD_CELLS).
    ...
    expect((app as any).clipboardCopyContext).toBeNull();

    app.destroy();
    root.remove();
  });

  it("does not cut (or clear) very large selections", async () => {
    const root = createRoot();
    const status = {
      activeCell: document.createElement("div"),
      selectionRange: document.createElement("div"),
    ...
    const clearRange = vi.spyOn((app as any).document, "clearRange");
