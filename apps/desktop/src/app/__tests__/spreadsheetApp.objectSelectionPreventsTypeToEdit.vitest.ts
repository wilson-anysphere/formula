...
describe("SpreadsheetApp object selection blocks type-to-edit", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  it("starts editing when typing with no object selected", () => {
  ...
  it("does not start editing when a chart is selected (canvas charts)", () => {
    const root = createRoot();
    const status = {
      activeCell: document.createElement("div"),
      selectionRange: document.createElement("div"),
