...
describe("SpreadsheetApp clipboard shortcuts (canvas charts)", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  it("does not copy the active cell range when a chart is selected", async () => {
  ...
  it("does not cut/clear the active cell range when a chart is selected", async () => {
    const root = createRoot();
    const status = {
      activeCell: document.createElement("div"),
      selectionRange: document.createElement("div"),
