...
describe("SpreadsheetApp undo/redo sheet fallback activation", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  it("activates the adjacent visible sheet when undo hides the active sheet", () => {
    ...
    try {
      ...
      // Excel-like: hide active sheet -> activate next visible sheet to the right.
  ...
  it("activates the adjacent visible sheet when redo deletes the active sheet", () => {
    const app = new SpreadsheetApp(createRoot(), createStatus());
    try {
      seedThreeSheets(app);
      const doc = app.getDocument();
      ...
      // Undo the delete so redo can delete it again while it's the active sheet.
      expect(app.undo()).toBe(true);

      app.activateSheet("Sheet2");
      expect(app.getCurrentSheetId()).toBe("Sheet2");
      expect(app.redo()).toBe(true);
      // Excel-like: delete active sheet -> activate next visible sheet to the right.
      expect(app.getCurrentSheetId()).toBe("Sheet3");
    } finally {
      app.destroy();
    }
