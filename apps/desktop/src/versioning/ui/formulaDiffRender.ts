...
export type FormulaDiffRenderOp = {
  type: DiffOpType;
  text: string;
};

export function isEffectivelyEmptyFormula(formula: string | null): boolean {
...
function nextTokenAcrossOps(ops: DiffOp[], opIndex: number, tokenIndex: number): Token | null {
  const op = ops[opIndex];
  if (!op) return null;
  const nextInOp = op.tokens[tokenIndex + 1];
  if (nextInOp) return nextInOp;
...
export function diffFormulaToRenderOps(before: string | null, after: string | null): FormulaDiffRenderOp[] {
  const { ops } = diffFormula(before, after) as { equal: boolean; ops: DiffOp[] };

  /** @type {FormulaDiffRenderOp[]} */
  const renderedOps: FormulaDiffRenderOp[] = [];
  ...
  for (let opIndex = 0; opIndex < ops.length; opIndex += 1) {
    ...
    for (let tokenIndex = 0; tokenIndex < op.tokens.length; tokenIndex += 1) {
      ...
      const nextToken = nextTokenAcrossOps(ops, opIndex, tokenIndex);

      if (shouldInsertLeadingSpace(prevPrevToken, prevToken, token)) {
        text += " ";
      }
      text += tokenToText(token, nextToken);

      prevPrevToken = prevToken;
      prevToken = token;
    }

    renderedOps.push({ type: op.type, text });
  }
