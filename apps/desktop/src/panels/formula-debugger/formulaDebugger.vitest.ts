...
import { buildStepTree, DebuggerState, flattenVisibleSteps } from "./steps";
import { highlightsForStep } from "./highlight";
import { explainError } from "./errorExplanation";
import type { TraceNode } from "./types";

describe("formula debugger", () => {
  it("e2e: open formula debugger for VLOOKUP and inspect steps + hover highlights", () => {
  ...
  it("error explanation uses trace context when available", () => {
    const formula = "=VLOOKUP(A1,B1:C2,2,FALSE)";
    const trace: TraceNode = {
      kind: { type: "function_call", name: "VLOOKUP" },
      span: { start: 1, end: formula.length },
    ...
    expect(explanation?.problem.toLowerCase()).toContain("not found");
    expect(explanation?.problem).toContain("B1:C2");
  });
});
