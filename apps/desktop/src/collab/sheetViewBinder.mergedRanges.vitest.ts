...
describe("bindSheetViewToCollabSession (mergedRanges)", () => {
  ...
  it("hydrates mergedRanges from the snake_case merged_cells key and reacts to nested view updates", () => {
    const doc = new Y.Doc();
    const sheets = doc.getArray<Y.Map<any>>("sheets");
    ...
    doc.transact(() => {
      viewMap.set("merged_cells", [{ start_row: 0, end_row: 1, start_col: 0, end_col: 2 }]);
    });

    expect(document.getMergedRanges(sheetId)).toEqual([{ startRow: 0, endRow: 1, startCol: 0, endCol: 2 }]);
