...
describe("createRibbonActionsFromCommands", () => {
  ...
  it("suppresses follow-up onCommand calls scheduled in a microtask after onToggle", async () => {
    const registry = new CommandRegistry();
    const run = vi.fn();
    registry.registerBuiltinCommand("ribbon.toggle.microtask", "Toggle", run);

    const actions = createRibbonActionsFromCommands({ commandRegistry: registry });
    actions.onToggle?.("ribbon.toggle.microtask", true);
    // Some hosts may schedule the follow-up `onCommand` asynchronously via microtasks.
    queueMicrotask(() => actions.onCommand?.("ribbon.toggle.microtask"));
    await flushMicrotasks();
  ...
  it("suppresses follow-up onCommand calls scheduled in a timer after onToggle", async () => {
    const registry = new CommandRegistry();
    const run = vi.fn();
    registry.registerBuiltinCommand("ribbon.toggle.timer", "Toggle", run);

    const actions = createRibbonActionsFromCommands({ commandRegistry: registry });
    actions.onToggle?.("ribbon.toggle.timer", true);
    // Some hosts may schedule the follow-up `onCommand` asynchronously via timers.
    await new Promise<void>((resolve) => {
      setTimeout(() => {
        actions.onCommand?.("ribbon.toggle.timer");
        resolve();
      }, 0);
    });
    await flushMicrotasks();
  ...
  it("suppresses follow-up onCommand calls scheduled via nested microtasks that then queue a timer after onToggle", async () => {
    const registry = new CommandRegistry();
    const run = vi.fn();
    registry.registerBuiltinCommand("ribbon.toggle.nestedMicrotaskTimer", "Toggle", run);

    const actions = createRibbonActionsFromCommands({ commandRegistry: registry });
    actions.onToggle?.("ribbon.toggle.nestedMicrotaskTimer", true);
    await new Promise<void>((resolve) => {
      queueMicrotask(() => {
        queueMicrotask(() => {
          setTimeout(() => {
            actions.onCommand?.("ribbon.toggle.nestedMicrotaskTimer");
            resolve();
          }, 0);
        });
      });
