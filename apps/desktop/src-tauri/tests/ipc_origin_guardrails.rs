...
fn assert_ipc_origin_checks(src: &str, file: &str, fn_name: &str) {
    ...
    let has_combined_stable = contains_call(window, "ensure_main_window_and_stable_origin");
    let has_combined_trusted = contains_call(window, "ensure_main_window_and_trusted_origin");
    let has_main = has_combined_stable || has_combined_trusted || contains_call(window, "ensure_main_window");
    let has_stable_origin = has_combined_stable || contains_call(window, "ensure_stable_origin");

    assert!(
        has_main,
        "{file}:{fn_name} is missing a main-window guard (expected `ipc_origin::ensure_main_window(...)`, `ipc_origin::ensure_main_window_and_trusted_origin(...)`, or `ipc_origin::ensure_main_window_and_stable_origin(...)`)"
    );
    assert!(
        has_stable_origin,
