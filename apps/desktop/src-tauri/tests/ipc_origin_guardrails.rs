...
fn assert_ipc_origin_checks(src: &str, file: &str, fn_name: &str) {
    ...
    let has_trusted = has_combined || contains_call(window, "ensure_trusted_origin");

    assert!(
        has_main,
        "{file}:{fn_name} is missing a main-window guard (expected `ipc_origin::ensure_main_window(...)` or `ipc_origin::ensure_main_window_and_trusted_origin(...)`)"
    );
    assert!(
        has_trusted,
        "{file}:{fn_name} is missing a trusted-origin guard (expected `ipc_origin::ensure_trusted_origin(...)` or `ipc_origin::ensure_main_window_and_trusted_origin(...)`)"
    );
}
