...
fn assert_ipc_origin_checks(src: &str, file: &str, fn_name: &str) {
    ...
    assert!(
        has_origin,
        "{file}:{fn_name} is missing an origin guard (expected `ipc_origin::ensure_trusted_origin(...)`, `ipc_origin::ensure_stable_origin(...)`, `ipc_origin::ensure_main_window_and_trusted_origin(...)`, or `ipc_origin::ensure_main_window_and_stable_origin(...)`)"
    );
}
