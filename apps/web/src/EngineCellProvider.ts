...
export interface EngineClientLike {
  getRange(range: string, sheet?: string): Promise<EngineCellLike[][]>;
}
...
function extractScalar(cell: EngineCellLike): EngineScalar {
  if (cell == null) return null;
  if (typeof cell === "object") {
    // The engine typically returns objects like `{ address, input, value }`.
    if ("value" in cell) return (cell as { value: EngineScalar }).value ?? null;
    return null;
  }
  return cell;
}
    ...
    try {
      ...
      for (let r = 0; r < rowCount; r++) {
        ...
        for (let c = 0; c < colCount; c++) {
          const scalar = extractScalar(cols[c]);
          const value = toGridValue(scalar);
