...
try {
  ...
  $sourceParts.Add("Microsoft Excel ($LocaleId)") | Out-Null
  if ($excelVersion) { $sourceParts.Add("version=$excelVersion") | Out-Null }
  if ($excelBuild) { $sourceParts.Add("build=$excelBuild") | Out-Null }
  if ($excelUiLocale) { $sourceParts.Add("uiLocale=$excelUiLocale") | Out-Null }

  $payload = [ordered]@{
    source = ($sourceParts -join " ") + " function name translations via Range.Formula/FormulaLocal round-trip (generated by tools/excel-oracle/extract-function-translations.ps1)."
