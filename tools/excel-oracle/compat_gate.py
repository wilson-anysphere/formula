...
def _redact_path_str(path: Path, *, privacy_mode: str, repo_root: Path | None) -> str:
    """Return a privacy-safe string representation of a filesystem path.

    In `privacy_mode=private`, hash absolute paths (which can include usernames/mount points on
    self-hosted runners). Keep repo-relative paths readable when possible.
    """
...
def _print_expected_dataset_synthetic_warning(
    *,
    expected_path: Path,
    info: _ExpectedDatasetInfo,
    ...
    msg_lines = [
        ...
        f"Path: {_redact_path_str(expected_path, privacy_mode=privacy_mode, repo_root=repo_root)}",
        "",
        "syntheticSource:",
        f"  kind:    {kind}",
        f"  version: {version}",
...
def main() -> int:
    ...
    print(
        "Excel-oracle datasets: "
        f"expected={_redact_path_str(expected_path, privacy_mode=args.privacy_mode, repo_root=repo_root)} "
        f"actual={_redact_path_str(actual_path, privacy_mode=args.privacy_mode, repo_root=repo_root)} "
        f"report={_redact_path_str(report_path, privacy_mode=args.privacy_mode, repo_root=repo_root)}",
        flush=True,
    )
    expected_info = _inspect_expected_dataset(expected_path)
    if expected_info is not None:
        if expected_info.is_synthetic:
            _print_expected_dataset_synthetic_warning(
    ...
    if args.dry_run:
        ...
        print(f"cases: {_redact_path_str(cases_path, privacy_mode=args.privacy_mode, repo_root=repo_root)}")
        print(
            f"expected: {_redact_path_str(expected_path, privacy_mode=args.privacy_mode, repo_root=repo_root)}"
        )
        print(f"actual: {_redact_path_str(actual_path, privacy_mode=args.privacy_mode, repo_root=repo_root)}")
        print(f"report: {_redact_path_str(report_path, privacy_mode=args.privacy_mode, repo_root=repo_root)}")
    ...
    try:
        ...
        lines.append("")
        cases_path_out = summary.get("casesPath") if isinstance(summary, dict) else None
        expected_path_out = summary.get("expectedPath") if isinstance(summary, dict) else None
        actual_path_out = summary.get("actualPath") if isinstance(summary, dict) else None

        lines.append(
            "* Cases corpus: `"
            + (
                cases_path_out
                if isinstance(cases_path_out, str) and cases_path_out
                else _redact_path_str(cases_path, privacy_mode=args.privacy_mode, repo_root=repo_root)
            )
            + "`"
        )
        lines.append(
            ...
            + (
                expected_path_out
                if isinstance(expected_path_out, str) and expected_path_out
                else _redact_path_str(expected_path, privacy_mode=args.privacy_mode, repo_root=repo_root)
            )
            + "`"
        )
        lines.append(
            ...
            + (
                actual_path_out
                if isinstance(actual_path_out, str) and actual_path_out
                else _redact_path_str(actual_path, privacy_mode=args.privacy_mode, repo_root=repo_root)
            )
            + "`"
        )
        lines.append(
            "* Report: `"
            + _redact_path_str(report_path, privacy_mode=args.privacy_mode, repo_root=repo_root)
            + "`"
        )
