...
class UpdatePinnedDatasetTests(unittest.TestCase):
    ...
    def test_records_real_excel_patch_metadata_sanitizes_windows_case_set_path(self) -> None:
        update = self._load_update_module()
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Windows absolute path should be sanitized before being written into source.patches.
            raw_subset = r"C:\Users\Alice\repo\tools\excel-oracle\odd_coupon_long_stub_cases.json"
            merge_path = tmp / "merge.json"
            merge_payload = {
                "schemaVersion": 1,
                "generatedAt": "2026-01-02T00:00:00Z",
                ...
                "caseSet": {"path": raw_subset, "sha256": "deadbeef", "count": 1},
                "results": [{"caseId": "case1", "result": {"t": "n", "v": 3}}],
            }
            ...
            pinned_updated = json.loads(pinned_path.read_text(encoding="utf-8"))
            patches = (pinned_updated.get("source") or {}).get("patches")
            self.assertIsInstance(patches, list)
            self.assertEqual(len(patches), 1)
            patch_case_path = (patches[0].get("caseSet") or {}).get("path")
            self.assertEqual(patch_case_path, "tools/excel-oracle/odd_coupon_long_stub_cases.json")
            self.assertNotIn("Alice", json.dumps(pinned_updated))
