...
class PinDatasetTests(unittest.TestCase):
    ...
    def test_normalizes_absolute_case_set_path(self) -> None:
        pin_py = Path(__file__).resolve().parents[1] / "pin_dataset.py"
        self.assertTrue(pin_py.is_file(), f"pin_dataset.py not found at {pin_py}")
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Simulate a dataset produced on Windows with an absolute cases path that includes a
            # username/mount point.
            raw_cases_path = r"C:\Users\Alice\repo\tests\compatibility\excel-oracle\cases.json"
            dataset_payload = {
                "schemaVersion": 1,
                "generatedAt": "unit-test",
                "source": {
