...
class OddCouponBoundarySubsetSyncTests(unittest.TestCase):
    def test_boundary_subset_file_matches_canonical_boundary_tag(self) -> None:
        """
        The repo keeps a small convenience corpus for quickly running odd-coupon boundary
        ...
        boundary_cases = [
            ...
            and "boundary" in c["tags"]
        ]
        self.assertTrue(boundary_cases, "Expected at least one odd_coupon+boundary case in cases.json")

        expected_ids = {c.get("id") for c in boundary_cases if isinstance(c.get("id"), str)}
        subset_ids = {c.get("id") for c in subset_cases if isinstance(c.get("id"), str)}
        self.assertEqual(
            subset_ids,
            expected_ids,
            "Boundary subset file must contain exactly the odd_coupon+boundary case IDs from cases.json",
        )
        ...
        for case in subset_cases:
            ...
            self.assertIn("boundary", subset_tags, f"subset case is missing boundary tag: {cid!r}")
            self.assertTrue(
