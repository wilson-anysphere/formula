...
class OddCouponInvalidScheduleCorpusTests(unittest.TestCase):
    def test_invalid_schedule_cases_exist_in_canonical_corpus(self) -> None:
        ...
        for func in ("ODDFPRICE", "ODDFYIELD", "ODDLPRICE", "ODDLYIELD"):
            matches = [
                ...
                and has_all_tags(c, "financial", "odd_coupon", "invalid_schedule", func)
            ]
            self.assertTrue(
                matches,
                f"Missing odd-coupon invalid schedule oracle coverage: tags financial+odd_coupon+invalid_schedule+{func}",
            )

    def test_subset_corpus_ids_and_formulas_match_canonical(self) -> None:
        repo_root = Path(__file__).resolve().parents[3]
        cases_path = repo_root / "tests/compatibility/excel-oracle/cases.json"
        subset_path = repo_root / "tools/excel-oracle/odd_coupon_invalid_schedule_cases.json"
        self.assertTrue(cases_path.is_file(), f"cases.json not found at {cases_path}")
