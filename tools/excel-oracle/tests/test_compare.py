...
class ComparePrivacyModeTests(unittest.TestCase):
    ...
    def test_privacy_mode_redacts_path_keys_inside_value_objects(self) -> None:
        compare_py = Path(__file__).resolve().parents[1] / "compare.py"
        self.assertTrue(compare_py.is_file(), f"compare.py not found at {compare_py}")
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            secret_path = r"C:\Users\Alice\secret.xlsx"
            actual_payload = {
                "schemaVersion": 1,
                "generatedAt": "unit-test",
                "source": {"kind": "engine"},
                ...
                "results": [
                    {
                        "caseId": "case-a",
                        "result": {"t": "e", "v": "#VALUE!", "path": secret_path},
                    }
                ],
            }
            actual_path.write_text(
            ...
            actual_value = mismatches[0].get("actual")
            self.assertIsInstance(actual_value, dict)
            expected_hash = hashlib.sha256(secret_path.encode("utf-8")).hexdigest()
            self.assertEqual(actual_value.get("path"), f"sha256={expected_hash}")
            self.assertNotIn(secret_path, json.dumps(report))

    def test_privacy_mode_hashes_extra_error_string_fields(self) -> None:
        compare_py = Path(__file__).resolve().parents[1] / "compare.py"
        self.assertTrue(compare_py.is_file(), f"compare.py not found at {compare_py}")
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            secret_message = "Failed to open C:\\Users\\Alice\\secret.xlsx"
            actual_payload = {
                "schemaVersion": 1,
                "generatedAt": "unit-test",
                "source": {"kind": "engine"},
                ...
                "results": [
                    {
                        "caseId": "case-a",
                        "result": {"t": "e", "v": "#VALUE!", "message": secret_message},
                    }
                ],
            }
            actual_path.write_text(
            ...
            actual_value = mismatches[0].get("actual")
            self.assertIsInstance(actual_value, dict)
            self.assertEqual(actual_value.get("v"), "#VALUE!")
            expected_hash = hashlib.sha256(secret_message.encode("utf-8")).hexdigest()
            self.assertEqual(actual_value.get("message"), f"sha256={expected_hash}")
            self.assertNotIn(secret_message, json.dumps(report))
