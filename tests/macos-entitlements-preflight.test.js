...
test("macOS entitlements preflight ignores commented-out entitlements", async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "macos-entitlements-commented-fail-"));
  try {
    await writeFixtureFile(
      tmpRoot,
      "apps/desktop/src-tauri/tauri.conf.json",
...
test("macOS entitlements preflight requires network.server when App Sandbox is enabled", async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "macos-entitlements-sandbox-fail-"));
  try {
    await writeFixtureFile(
      tmpRoot,
      "apps/desktop/src-tauri/tauri.conf.json",
