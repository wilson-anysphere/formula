...
test("ExtensionHostManager.startup quarantines invalid integrity metadata instead of throwing", async (t) => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "formula-desktop-ext-invalid-meta-"));
  ...
  const badId = "formula.invalid-meta";
  // The `files` list contains an illegal path entry that should not crash the runtime.
  const badFiles = [{ path: "../evil", sha256: "0".repeat(64), size: 0 }];
    ...
    JSON.stringify(
      {
        installed: {
          ...
          [badId]: { id: badId, version: "1.0.0", installedAt: new Date().toISOString(), files: badFiles },
        },
      },
      null,
      2,
