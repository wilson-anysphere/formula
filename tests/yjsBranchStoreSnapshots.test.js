...
class CountingYjsBranchStore extends YjsBranchStore {
  applyPatchCalls = 0;
...
test("YjsBranchStore snapshotting bounds patch replay", async () => {
  const snapshotEveryNCommits = 10;
  const commits = 79;
  const ydoc = new Y.Doc();
  const store = new CountingYjsBranchStore({ ydoc, snapshotEveryNCommits });
