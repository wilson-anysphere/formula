...
test("binder: full-column format syncs via layered formats without per-cell materialization", async () => {
  const ydoc = new Y.Doc();
  const cells = ydoc.getMap("cells");
  const sheets = ydoc.getArray("sheets");

  // Seed a default Sheet1 entry so column default formats have a place to live.
  ydoc.transact(() => {
  ...
  try {
    // Apply a full-column format change. This should become a single column-default update,
    // not 1M per-cell deltas.
    docA.setRangeFormat("Sheet1", "A1:A1048576", { font: { bold: true } });
    ...
    // The Yjs cells map should stay empty (no per-cell materialization for the column style).
