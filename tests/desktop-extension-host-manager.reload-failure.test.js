...
test("ExtensionHostManager.reloadExtension emits contribution changes even when reload fails after unload", async (t) => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "formula-desktop-ext-reload-failure-"));
  ...
  // Tamper with the on-disk entrypoint so the integrity check fails on reload.
  await fs.appendFile(path.join(installedPath, "dist", "extension.js"), "\n// tampered\n", "utf8");

  await assert.rejects(() => runtime.reloadExtension(extensionId), /integrity check failed/i);

  assert.ok(snapshots.length >= 1);
  const last = snapshots[snapshots.length - 1];
  assert.ok(!last.commands.some((cmd) => cmd.command === "sampleHello.sumSelection"));
