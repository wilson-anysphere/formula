...
test("ExtensionHostManager.syncInstalledExtensions quarantines tampering and unloads the extension", async (t) => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "formula-desktop-ext-tamper-"));
  ...
  // Tamper with the installed bytes after the extension is already loaded.
  await fs.appendFile(path.join(installedPath, "dist", "extension.js"), "\n// tampered\n", "utf8");

  await runtime.syncInstalledExtensions();

  // The tampered extension should have been unloaded and quarantined.
  await assert.rejects(() => runtime.executeCommand("sampleHello.sumSelection"), /Unknown command/i);

  const updatedState = JSON.parse(await fs.readFile(statePath, "utf8"));
  assert.equal(updatedState.installed[extensionId].corrupted, true);
  assert.match(
    String(updatedState.installed[extensionId].corruptedReason ?? ""),
