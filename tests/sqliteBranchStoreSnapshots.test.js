...
class CountingSQLiteBranchStore extends SQLiteBranchStore {
  applyPatchCalls = 0;
...
test("SQLiteBranchStore snapshotting bounds patch replay", async () => {
  const snapshotEveryNCommits = 10;
  const commits = 79;
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "branch-store-snapshots-"));
  const storePath = path.join(tmpDir, "branches.sqlite");

  const store = new CountingSQLiteBranchStore({ filePath: storePath, snapshotEveryNCommits });
