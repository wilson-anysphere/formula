...
def brace_delta_with_state(line: str, st: BraceParseState) -> int:
    ...
    This keeps the cfg(test) skipper robust even when test helpers contain strings
    like `" }); "` or raw strings with lots of braces.
    """
    delta = 0
    i = 0
    n = len(line)
    ...
    while i < n:
        ...
        # Char literal / byte char literal. We only treat this as a char literal if we can
        # find the closing `'` shortly after; otherwise it's likely a lifetime like `'a`.
        if ch == "'":
            j = i + 1
            esc = False
            found_close = False
            ...
            # Treat as lifetime-ish. Don't enter a multi-line state machine here.
            i += 1
            continue
