...
function writePackageRoot(dir, { execLine, mimeTypeLine } = {}) {
  const pkgRoot = path.join(dir, "pkg");
  const applicationsDir = path.join(pkgRoot, "usr", "share", "applications");
  const docDir = path.join(pkgRoot, "usr", "share", "doc", "formula-desktop");
...
function runValidator({ packageRoot, configPath }) {
  const proc = spawnSync(
    "python3",
    [scriptPath, "--package-root", packageRoot, "--tauri-config", configPath],
