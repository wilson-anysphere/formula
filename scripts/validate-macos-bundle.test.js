...
test(
  "validate-macos-bundle fails when embedded entitlements enable a forbidden key",
  { skip: !hasBash },
  () => {
    const tmp = mkdtempSync(join(tmpdir(), "formula-macos-bundle-test-"));
    const binDir = join(tmp, "bin");
    ...
    writeFakeTool(
      binDir,
      "codesign",
      `#!/usr/bin/env bash\nset -euo pipefail\necho \"codesign $*\" >> \"${logPath}\"\ncase \"${"$"}*\" in\n  *\"--entitlements\"*)\n    echo \"Executable=${"$"}{4:-/path/to/formula-desktop}\" >&2\n    # Enabling disable-library-validation should be rejected by the guardrail.\n    cat >&2 <<'PLIST'\n<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">\n<plist version=\"1.0\">\n  <dict>\n    <key>com.apple.security.network.client</key>\n    <true/>\n    <key>com.apple.security.cs.allow-jit</key>\n    <true/>\n    <key>com.apple.security.cs.allow-unsigned-executable-memory</key>\n    <true/>\n    <key>com.apple.security.cs.disable-library-validation</key>\n    <true/>\n  </dict>\n</plist>\nPLIST\n    ;;\n  *\"-dv\"*)\n    echo \"Runtime Version=13.0.0\" >&2\n    echo \"CodeDirectory v=20500 size=0 flags=0x10000(runtime) hashes=0 location=embedded\" >&2\n    ;;\nesac\nexit 0\n`,
    );
    writeFakeTool(
      binDir,
      "spctl",
    ...
    assert.notEqual(proc.status, 0, "expected non-zero exit status");
    assert.match(`${proc.stdout}\n${proc.stderr}`, /disable-library-validation/i);
  },
);
